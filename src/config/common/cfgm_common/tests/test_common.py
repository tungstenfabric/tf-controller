...
def create_api_server_instance(test_id, config_knobs, db='cassandra'):
    ...
    # If the thread crashed because an exception occured, no need to
    # wait to stop the process.
    ret_server_info['greenlet'].link_exception(
        lambda glet: sys.exit("fatal exception occured when trying to "
                              "create api server instance: {}".format(
                                  glet.exception)))
...
class TestCase(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def create_network_policy(self, vn1, vn2, service_list=None, mirror_service=None,
        ...
        if retain_as_path is True and (mirror_service or service_list):
