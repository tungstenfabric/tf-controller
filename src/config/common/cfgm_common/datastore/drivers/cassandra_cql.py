...
# Set number of rows per pages fetched by get_range
GET_RANGE_PAGE_SIZE = 10000
...
class CassandraDriverCQL(datastore_api.CassandraDriver):
    ...
    def _cql_select(self, cf_name, keys, start='', finish='', num_columns=None,
        ...
        for key in keys:
            ...
            if num_columns and self.SupportsPerPartitionLimit:
                arg.append(num_columns)
            args.append(arg)

        req = self.apply(ses, pre, args)
        req = zip(keys, req)
        req = Iter(req,
    ...
    def _Get_Range(self, cf_name, columns=None,
        ...
        pre = ses.prepare(cql)
        sta = pre.bind(arg)
        sta.fetch_size = GET_RANGE_PAGE_SIZE

        current_key = None
        for row in ses.execute(sta):
            key, row = row[0], row[1:]
    ...
    def _Insert(self, key, columns, keyspace_name=None, cf_name=None,
        ...
        if batch is not None:
            for column, value in columns.items():
                batch.add_insert(key, pre, [StringType(key),
                                            StringType(column),
                                            StringType(value)])
        else:
            self._cql_execute(ses, pre, key, columns)

    def _cql_execute(self, ses, pre, key, columns):
        args = []
        if isinstance(columns, dict):
            ...
            for column in columns:
                args.append([StringType(key),
                             StringType(column)])
        self.apply(ses, pre, args)

    def apply(self, ses, pre, args):
        return connector.concurrent.execute_concurrent_with_args(
            ses, pre, args, concurrency=100, results_generator=False)
    ...
    def _Remove(self, key, columns=None, keyspace_name=None, cf_name=None,
        ...
        if not columns:
            ...
            pre = ses.prepare(cql)
            if batch is not None:
                batch.add_remove(key, pre, [StringType(key)])
            else:
                ses.execute(pre.bind([StringType(key)]))
        else:
            cql = """
              DELETE FROM "{}"
              WHERE key = textAsBlob(?)
