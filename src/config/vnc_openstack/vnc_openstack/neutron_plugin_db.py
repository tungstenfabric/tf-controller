...
from vnc_api.vnc_api import (
    ...
    DhcpOptionsListType, DhcpOptionType, Domain, FirewallPolicy, FirewallRule,
    FirewallRuleEndpointType, FirewallSequence, FirewallServiceType,
    FloatingIp, FloatingIpPool, IdPermsType, InstanceIp, InterfaceRouteTable,
    IpamSubnetType, IpamType, KeyValuePair, KeyValuePairs, LogicalRouter,
    MacAddressesType, NetworkIpam, NetworkPolicy, NoIdError, OverQuota,
    PermType2, PolicyEntriesType, PolicyRuleType, PortType, Project,
    ProviderDetails, RefsExistError, RouteTable, RouteTableType, RouteType,
    SecurityGroup, SequenceType, ServiceInstance, ServiceInstanceType,
    ServiceScaleOutType, SubnetType, Tag, VirtualMachine,
        ...
        if api_server_obj:
            self.api_server_routes = dict(
                (r.rule.split('/')[1] + '-' + r.method,
                 r.callback) for r in api_server_obj.api_bottle.routes)
...
class DBInterface(object):
    ...
    def __init__(self, manager, admin_name, admin_password, admin_tenant_name,
        ...
        self.lock_path_prefix = '%s/%s' % (
            self._api_server_obj._args.cluster_id,
            _DEFAULT_ZK_LOCK_PATH_PREFIX)
        self.security_group_lock_prefix = '%s/security_group' % \
                                          self.lock_path_prefix
        self.virtual_network_lock_prefix = '%s/virtual_network' % \
            self.lock_path_prefix
        self._zookeeper_client = \
            self._api_server_obj._db_conn._zk_db._zk_client
    ...
    def _get_resource_id(self, data_q, create_id=False):
        ...
        if resource_id is None and create_id:
    ...
    def _ensure_instance_exists(self, instance_id, tenant_id, baremetal=False):
        ...
        try:
            ...
            if baremetal and db_instance_obj.get_server_type() != \
                    "baremetal-server":
    ...
    def _security_group_rule_create(self, sg_id, sg_rule):
        ...
        try:
            ...
            try:
                ...
                scope_lock.release()
                self._raise_contrail_exception(
                    'BadRequest', resource='security_group_rule', msg=str(e))
            except OverQuota as e:
                scope_lock.release()
                self._raise_contrail_exception(
                    'OverQuota', overs=['security_group_rule'], msg=str(e))
            ...
            self._raise_contrail_exception(
                'BadRequest', resource='security_group_rule', msg=msg)
        finally:
            scope_lock.release()
    ...
    def tag_get(self, context, parent_id, tag):
        ...
        return json.dumps(tag_obj.fq_name[0].split('=')[1])
    ...
    def _resource_read(self, resource_uuid, **kwargs):
        ...
        if resource_type == 'subnet':
            return self.subnet_read(resource_uuid)

        read_method = getattr(self._vnc_lib, '{}_read'.format(resource_type))
    ...
    def tags_create_or_update(self, context, parent_id, tags):
        ...
        vnc_obj = self._resource_read(parent_id)
        if isinstance(vnc_obj, dict):
            self._subnet_add_tags(parent_id, tags)
        ...
        tags_dict = [{'fq_name': t.fq_name} for t in tags_obj]
        return json.dumps([self._tags_vnc_to_neutron(t) for t in tags_dict])
    ...
    def _floating_ip_pool_delete(self, fip_pool):
        if 'floating_ips' in fip_pool:
            ...
            fip_dict = self._vnc_lib.floating_ips_list(
                obj_uuids=fip_ids, fields=['virtual_machine_interface_refs'],
                detail=False)
    ...
    def _fip_pool_list_network(self, net_id, fields=None):
        resp_dict = self._vnc_lib.floating_ip_pools_list(
            parent_id=net_id, fields=fields)
    ...
    def _port_list(self, port_objs):
        ...
        iip_objs = self._instance_ip_list(
            back_ref_id=[port_obj.uuid for port_obj in port_objs])
        memo_req['instance-ips'] = dict((iip_obj.uuid, iip_obj)
        ...
        for port_obj in port_objs:
            if port_obj.parent_type == 'virtual-machine':
                ...
                vm_id = self._vnc_lib.fq_name_to_id(
                    'virtual-machine', port_obj.get_fq_name()[:-1])
                vm_ids.append(vm_id)
            else:
        ...
        memo_req['virtual-machines'] = dict((vm_obj.uuid, vm_obj)
        ...
        for port_obj in port_objs:
            port_info = self._port_vnc_to_neutron(
                port_obj, memo_req, oper=LIST)
    ...
    def _port_list_project(self, project_id, count=False, is_admin=False):
        if self._list_optimization_enabled:
            ...
            if count:
                ...
                ret_val = []
            net_objs = self._virtual_network_list(
                project_id, fields=['virtual_machine_interface_back_refs'],
                detail=True)
            if not net_objs:
                return ret_val
    ...
    def _network_neutron_to_vnc(self, network_q, oper):
        ...
        if oper == CREATE or oper == UPDATE:
            if 'port_security_enabled' in network_q:
                net_obj.set_port_security_enabled(
                    network_q['port_security_enabled'])
    ...
    def _network_vnc_to_neutron(self, net_obj, oper=READ, context=None):
        ...
        if net_obj.is_shared or (
                net_obj.perms2 and self._shared_with_tenant(
                context, net_obj.perms2.share)):
        ...
        if net_obj.provider_properties:
            net_q_dict[
                ...
                net_obj.provider_properties.physical_network
            net_q_dict[
                'provider:segmentation_id'] = \
                net_obj.provider_properties.segmentation_id
        ...
        net_q_dict[
            'port_security_enabled'] = net_obj.get_port_security_enabled()

        if net_obj.get_id_perms().get_description() is not None:
            net_q_dict['description'] = \
                net_obj.get_id_perms().get_description()
    ...
    def _subnet_neutron_to_vnc(self, subnet_q):
        ...
        dns_s_addr = '0.0.0.0' if self._strict_compliance else \
            dns_server_address
    ...
    def _router_vnc_to_neutron(self, rtr_obj, oper=READ):
        ...
        if rtr_obj.get_id_perms().get_description() is not None:
            rtr_q_dict['description'] = \
                rtr_obj.get_id_perms().get_description()
    ...
    def _floatingip_neutron_to_vnc(self, context, fip_q, oper):
        ...
        if port_id:
            try:
                ...
                if fip_proj_list:
                    fip_tenant_id = fip_proj_list[0].get(
                        'uuid').replace('-', '')
                    if port_tenant_id != fip_tenant_id:
                        msg = "port can't be associated with this fip as " \
                              "they both belong to different tenants"
                        self._raise_contrail_exception(
            ...
            if self._strict_compliance:
                ...
                pvt_net_obj = self._vnc_lib.virtual_network_read(
                    id=port_net_id)
                ...
                try:
                    ...
                    msg = "Network %s doesn't provide a floatingip pool" % \
                        port_net_id
                    self._raise_contrail_exception(
                        'BadRequest', resource="floatingip", msg=msg)
                try:
                    ext_vn_obj = self._vnc_lib.virtual_network_read(
                ...
                if logical_router_refs:
                    try:
                        for router_ref in (logical_router_refs):
                            ext_router_obj = self._vnc_lib.logical_router_read(
                                id=router_ref['uuid'])
                            ext_ports_uuid_list += [vmi['uuid'] for vmi in (
                                ext_router_obj.
                                get_virtual_machine_interface_refs() or [])]
                    except BaseException:
                        self._raise_contrail_exception(
                            'ExternalGatewayForFloatingIPNotFound',
                            subnet_id=pvt_subnet_id,
                ...
                pvt_port_objs = self._virtual_machine_interface_list(
                    back_ref_id=port_net_id)
                pvt_ports_uuid_list = [port_obj.get_uuid()
                                       for port_obj in pvt_port_objs]
    ...
    def _floatingip_vnc_to_neutron(self, fip_obj, memo_req=None, oper=READ):
        ...
        try:
            floating_net_id = memo_req['network_fqn'][tuple(
        ...
        if tenant_id and port_obj:
            ...
            for router_obj in router_list or []:
                vmi_routers.update(
                    dict((vmi_ref['uuid'], router_obj.uuid) for vmi_ref in (
                        router_obj.get_virtual_machine_interface_refs() or
                        [])))

            if memo_req:
                vmi_obj_list = [memo_req['ports'].get(
                    vmi_id) for vmi_id in vmi_routers]
            else:
                if vmi_routers:
    ...
    def _port_neutron_to_vnc(self, port_q, net_obj, oper):
        if oper == CREATE:
            ...
            if ('mac_address' in port_q and port_q['mac_address']):
                ...
                port_obj.set_virtual_machine_interface_mac_addresses(
                    mac_addrs_obj)
        else:  # READ/UPDATE/DELETE
            port_obj = self._virtual_machine_interface_read(
                port_id=port_q['id'])
        ...
        if (port_q.get('device_owner') != constants.DEVICE_OWNER_ROUTER_GW and
                port_q.get('device_owner') not in
                constants.ROUTER_INTERFACE_OWNERS_SNAT and
                'device_id' in port_q):
        ...
        if 'device_owner' in port_q:
            port_obj.set_virtual_machine_interface_device_owner(
                port_q.get('device_owner'))
        ...
        if oper == CREATE:
            ...
            if 'mac_address' in port_q and allowed_port:
                # Ensure that duplicate mac address does not exist on this
                # network
                ports = self._virtual_machine_interface_list(
                    back_ref_id=net_id)
                for port in ports:
                    ...
                    for mac in macs.get_mac_address():
                        ...
                        if mac == port_q['mac_address'] and \
                                port.uuid != port_q['id']:
                            raise self._raise_contrail_exception(
                                "MacAddressInUse", net_id=net_id,
                                mac=port_q['mac_address'])
        ...
        if oper == CREATE:
            ...
            if 'port_security_enabled' in port_q:
                port_obj.set_port_security_enabled(
                    port_q['port_security_enabled'])
            ...
            if port_security:
                if ('security_groups' in port_q and
                    ...
                    if not port_q['security_groups']:
                        ...
                        for sg_id in port_q.get('security_groups') or []:
                            try:
                                # TODO optimize to not read sg (only uuid/fqn
                                # needed)
                                sg_obj = self._vnc_lib.security_group_read(
                                    id=sg_id)
                ...
                if port_sg_refs:
                    if 'security_groups' in port_q and not port_q[
                            'security_groups']:
            ...
            for fixed_ip in port_q.get('fixed_ips', []):
                if 'ip_address' in fixed_ip:
                    ...
                    if port_obj_ips is None:
                        port_obj_ips = []
                        ip_back_refs = getattr(
                            port_obj, 'instance_ip_back_refs', None)
                        if ip_back_refs:
    ...
    def _gw_port_vnc_to_neutron(self, port_obj, port_req_memo, oper=READ):
        ...
        try:
            ...
            try:
                si_obj = self._vnc_lib.service_instance_read(
                    id=si_refs[0]['uuid'], fields=["logical_router_back_refs"])
            except NoIdError:
                return None
    ...
    def _port_get_interface_status(self, port_obj):
        ...
        # if parent interface of a sub interface is attached to a VM,
        # then subinterface
        # is in PORT_STATUS_ACTIVE .
        parent_port_obj = self._virtual_machine_interface_read(
            port_id=vmi_refs[0]['uuid'], fields=['virtual_machine_refs'])
    ...
    def _port_vnc_to_neutron(self, port_obj, port_req_memo=None, oper=READ):
        ...
        dhcp_options_list = \
            port_obj.get_virtual_machine_interface_dhcp_option_list()
        ...
        allowed_address_pairs = \
            port_obj.get_virtual_machine_interface_allowed_address_pairs()
        address_pairs = []
        if allowed_address_pairs and \
            ...
            for aap in allowed_address_pairs.allowed_address_pair:
                ...
                if IPNetwork(aap.ip.get_ip_prefix()).version == 4:
                    ip_len = 32
                elif IPAddress(aap.ip.get_ip_prefix()).version == 6:
                    ip_len = 128
        ...
        if ip_back_refs:
            for ip_back_ref in ip_back_refs:
                ...
                if ip_obj.get_service_health_check_ip():
                    primary_ip = False
                    try:
                        extra_dict['service_health_check_ips'].append(
                            ip_q_dict)
                    except KeyError:
                ...
                if ip_obj.get_service_instance_ip():
                    ...
                    # type v1 service_instance and service_instance_ip has to
                    # be added in the fixed_ip list
        ...
        if router_refs is not None:
            ...
            if rtr_uuid:
                ...
                # https://github.com/openstack/neutron/blob/master/neutron/db
                # /l3_db.py#L354-L355
        ...
        port_q_dict[
            'port_security_enabled'] = port_obj.get_port_security_enabled()
                ...
                for ip_addr in [fixed_ip['ip_address'] for fixed_ip in
                    ...
                    if ip_addr in host_prefixes:
                        self._port_add_iface_route_table(
                            host_prefixes[ip_addr], port_obj, sn_id)
    ...
    def _port_add_iface_route_table(self, route_prefix_list, port_obj,
        ...
        try:
            ...
            intf_route_table_id = self._resource_create(
                'interface_route_table', intf_route_table)
            intf_route_table_obj = self._vnc_lib.interface_route_table_read(
    ...
    def network_delete(self, net_id, context):
        ...
        try:
            fip_pools = self._fip_pool_list_network(
                net_id, fields=['floating_ips'])
    ...
    def network_list(self, context=None, filters=None):
        ...
        if context and not context['is_admin']:
            ...
            # and prune the return list with shared = False or  shared = None
                ...
                if 'shared' in filters and filters['shared'][0] is False:
        ...
        # admin role from here on
            ...
            if 'shared' in filters:
                shared = filters['shared'][0]
            nets = self._network_list_filter(
                shared=shared,
                router_external=router_external)
            for net in nets:
                net_info = self._network_vnc_to_neutron(
    ...
    def with_zookeper_vn_lock(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                # If the lock was not acquired and timeout of 5 seconds
                # happened, then raise
                # a bad request error.
                self._api_server_obj.config_log(
                    "Zookeeper lock acquire timed out.",
    ...
    def _subnet_update(self, net_id, subnet_id, subnet_q):
        ...
        if ipam_refs:
            for ipam_ref in ipam_refs:
                ...
                if subnet_found:
                    if 'name' in subnet_q:
                        if subnet_q['name'] is not None:
                            subnet_vnc.set_subnet_name(subnet_q['name'])
                    if 'gateway_ip' in subnet_q:
                        if subnet_q['gateway_ip'] is not None:
                            subnet_vnc.set_default_gateway(
                                subnet_q['gateway_ip'])

                    if 'enable_dhcp' in subnet_q:
                        if subnet_q['enable_dhcp'] is not None:
                            subnet_vnc.set_enable_dhcp(subnet_q['enable_dhcp'])

                    if 'dns_nameservers' in subnet_q:
                        if subnet_q['dns_nameservers'] is not None:
                            dhcp_options = []
                            dns_servers = " ".join(subnet_q['dns_nameservers'])
                            if dns_servers:
                                dhcp_options.append(
                            ...
                            if dhcp_options:
                                subnet_vnc.set_dhcp_option_list(
                                    DhcpOptionsListType(dhcp_options))
                            else:
                                subnet_vnc.set_dhcp_option_list(None)

                    if 'host_routes' in subnet_q:
                        if subnet_q['host_routes'] is not None:
                            host_routes = []
                            for host_route in subnet_q['host_routes']:
                                host_routes.append(
                                    RouteType(
                            ...
                            if self._apply_subnet_host_routes:
                                ...
                                subnet_cidr = '%s/%s' % (
                                    subnet_vnc.subnet.get_ip_prefix(),
                                    subnet_vnc.subnet.get_ip_prefix_len())
                                self._port_update_iface_route_table(
                                    net_obj, subnet_cidr, subnet_id,
                                    host_routes, old_host_routes)
                            if host_routes:
                                subnet_vnc.set_host_routes(
                    ...
                    for subnet_vnc in subnet_vncs:
                        ...
                        if (filters and 'shared' in filters and
                            ...
                            if not self._filters_is_present(
                                ...
                                continue
                            if not self._filters_is_present(
                                    filters, 'tenant_id', str(
                                        uuid.UUID(sn_proj_id))):
                                continue
                            if not self._filters_is_present(
                ...
                try:
                    ...
                    if not net_obj.get_router_external():
                        self._raise_contrail_exception(
                            'BadRequest',
                            resource='router',
                            msg="Network %s is not a valid external network" %
                                network_id)
                except NoIdError:
                    self._raise_contrail_exception('NetworkNotFound',
                                                   net_id=network_id)
    ...
    def remove_router_interface(self, router_id, port_id=None, subnet_id=None):
        ...
        if port_id:
            ...
            if (port_db['device_owner'] not in
                    [constants.DEVICE_OWNER_ROUTER_INTF,
                     constants.DEVICE_OWNER_DVR_INTERFACE] or
    ...
    def floatingip_list(self, context, filters=None):
        ...
        memo_req['ports'] = dict((vmi_obj.uuid, vmi_obj)
    ...
    def port_list(self, context=None, filters=None):
        ...
        for neutron_port in neutron_ports:
            ...
            if ('fixed_ips' in filters and not self._port_fixed_ips_is_present(
                    filters['fixed_ips'], neutron_port['fixed_ips'])):
                continue
    ...
    def security_group_create(self, sg_q):
        ...
        # security_group_read will internally call
    ...
    """Firewall as a Service v2 Contrail mapping
        ...
        https://docs.openstack.org/neutron/latest/admin/fwaas.html#fwaas-v2
        Contrail mapping spec:
        https://github.com/Juniper/contrail-specs/blob/master/neutron_FWaaSv2.md
    """
    ...
    def _firewall_group_neutron_to_vnc(self, context, firewall_group, id=None):
        ...
        if ingress != attr_not_specified or egress != attr_not_specified:
            if (ingress not in [None, attr_not_specified] and
                    ...
                    ingress != egress):
                msg = (
                    "Contrail firewall policies does not distinguish "
    ...
    def firewall_group_create(self, context, firewall_group):
        ...
        try:
            aps_id = self._resource_create('application_policy_set', aps)
        except BadRequest as e:
            self._raise_contrail_exception(
                'BadRequest', resource='firewall_group', msg=str(e))
        get_context().push_undo(
            self._vnc_lib.application_policy_set_delete, id=aps_id)
        ...
        aps = self._vnc_lib.application_policy_set_read(id=aps_id)
    ...
    def firewall_group_list(self, context, filters=None, fields=None):
        ...
        apss = self._vnc_lib.application_policy_sets_list(
            ...
            back_ref_id=(filters.pop('ingress_firewall_policy_id', []) +
                         filters.pop('egress_firewall_policy_id', [])) or
    ...
    def firewall_policy_create(self, context, firewall_policy):
        ...
        try:
            fp_id = self._resource_create('firewall_policy', fp)
        except BadRequest as e:
            self._raise_contrail_exception(
    ...
    def firewall_rule_create(self, context, firewall_rule):
        ...
        try:
            fr_id = self._resource_create('firewall_rule', fr)
        except BadRequest as e:
            self._raise_contrail_exception(
            ...
            if 'sub_ports' in trunk_q:
                ...
                if port_operation == 'ADD_SUBPORTS':
                    ...
                    if sub_vmi_uuids:
                        ...
                        for vmi in sub_vmis:
                            prop = \
                                vmi.get_virtual_machine_interface_properties()
                            if prop:
        ...
        if trunk_obj.get_virtual_machine_interface_refs():
            for sub_port in trunk_obj.get_virtual_machine_interface_refs():
                ...
                if sub_port['uuid'] == port_id:
                    continue
                sub_port_obj = self._vnc_lib.virtual_machine_interface_read(
                    id=sub_port['uuid'])

                vmi_prop = \
                    sub_port_obj.get_virtual_machine_interface_properties()
                vlan_tag = vmi_prop.get_sub_interface_vlan_tag() if vmi_prop \
                    else None
