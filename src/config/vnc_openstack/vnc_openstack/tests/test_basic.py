...
class TestBasic(test_case.NeutronBackendTestCase):
    ...
    def test_list_with_inconsistent_members(self):
        ...
        objects['virtual_machine_interface'] = [
            vnc_api.VirtualMachineInterface(
                '%s-%s' %
                (self.id(), i), proj_obj) for i in range(3)]
    ...
    def test_baremetal_bindings_with_vpg_and_multi_vlan(self):
        ...
        for i in range(num_phy_interfaces + 1):
            ...
            mac = vnc_api.MacAddressesType(
                mac_address=['00:01:00:00:0f:c' + str(i)])
            pi = vnc_api.PhysicalInterface(
                name=pi_name,
                parent_obj=pr_obj,
                physical_interface_mac_addresses=mac)
            ...
            profile = {
                'port_id': pi_fq_name[i][2],
                'switch_id': pi_fq_name[i][2],
                'switch_info': pi_fq_name[i][1],
                'fabric': fabric_fq_name[1]}
            if i == num_phy_interfaces:
                binding_profile_update['local_link_information'].append(
                    profile)
            else:
    ...
    def test_virtual_port_group_physical_interface_vpg_id_association(self):
        ...
        # Make sure if VMI mac-address is all-zeroes
        self.assertEqual(port_dict.get('mac_address'), '00:00:00:00:00:00')

        # create baremetal bindings with another VMI and the
        # existing vpg through http_update() api call
        data2 = {'binding:profile': binding_profile}
        self.update_resource(
            'port',
            port_dict['id'],
            proj_uuid,
            extra_res_fields=data2)
        ...
        vpg_obj = self._vnc_lib.virtual_port_group_read(id=vpg_obj.uuid)
        vmi_refs = vpg_obj.get_virtual_machine_interface_refs()
        self.assertEqual(len(vmi_refs), 1)

        data3 = {'binding:profile': {},
                 'binding:host_id': ''}
        self.update_resource(
            'port',
            port_dict['id'],
        ...
        self._vnc_lib.virtual_network_delete(id=vn_obj.uuid)
        self._vnc_lib.virtual_network_delete(id=vn2_obj.uuid)
        self._vnc_lib.fabric_delete(id=fabric_uuid)
    # end test_virtual_port_group_physical_interface_vpg_id_association

    def test_virtual_port_group_ironic_workflow(self):
        """UT to verify Ironic Workflow - CEM-21992.

        Step 1: Create VMI without binding profile
        VMI create should be successful with zero mac-address
        Step 2: Update VMI with binding profile
        VMI update will create internal VPG and associate the PIs
        Step 3: Update VMI with empty binding profile
        Detach PIs from VPG and VPG still exists
        Step 4: Rerun Step 3
        Step 5: Create new VMI with same PIs when old VMI exists
        VMI create should be successful.
        Make sure new internal VPG gets created.
        Step 6: Delete both VMIs
        It should trigger internal VPGs deletion
        """
        mock_zk = self._api_server._db_conn._zk_db
        vn_obj = vnc_api.VirtualNetwork(self.id())
        vn_obj.add_network_ipam(vnc_api.NetworkIpam(),
        ...
        # Case 1:
        # Create a VMI without a binding profile
        # Expectation:
        # API-server should allow creating VMI without
        # profile (link local info) but No VPGs are created
        context = {'operation': 'CREATE',
                   'user_id': '',
                   'is_admin': True,
                   'roles': ''}
        ...
        body = {'context': context, 'data': data}
        resp = self._api_svr_app.post_json('/neutron/port', body)
        port_1_dict = json.loads(resp.text)

        # Make sure if VMI mac-address is all-zeroes
        self.assertEqual(port_1_dict.get('mac_address'), '00:00:00:00:00:00')

        # Make sure VPG is not created
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict.get('virtual-port-groups')
        self.assertTrue(len(vpgs) == 0)
        ...
        # Case 2:
        # Update VMI with binding profile
        # Expectation:
        # API-server would consume profile and create VPG
        # and associate PIs to it
        # update port with binding profile
        self.update_resource(
            'port', port_1_dict['id'], proj_uuid,
            extra_res_fields={'binding:profile': binding_profile})

        # make sure LAG is created
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict.get('virtual-port-groups')
        self.assertTrue(len(vpgs) == 1)
        self.assertTrue(vpgs[0]['fq_name'][-1] == 'vpg-internal-0')
        ...
        self.assertTrue(len(vpg_data.get_physical_interface_refs()) == 2)

        # Case 3:
        # Update VMI with an empty profile
        # This simulates ironic behavior where it conveys API-server
        # to de-associate PIs from VPG so it create new VMI with those
        # PIs
        # Expectation:
        # API-server will handle the empty profile and remove existing
        # PIs from VPG. but VPG will be deleted only when VMI is removed

        # update port with profile = '{}'
        empty_profile = '{}'
        self.update_resource(
            'port', port_1_dict['id'], proj_uuid,
            extra_res_fields={'binding:profile': empty_profile})

        # make sure LAG exists, but all PIs are removed
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict.get('virtual-port-groups')
        self.assertTrue(len(vpgs) == 1)

        # verify PIs are removed
        vpg_data = self._vnc_lib.virtual_port_group_read(id=vpgs[0]['uuid'])
        self.assertTrue(vpg_data.get_physical_interface_refs() is None)
        ...
        # Case 4:
        # Create a new VMI with same PIs when old VMI exists
        # Expectation:
        # API-server will creating new VMI with same PIs, create
        # another internal VPG, associate PIs to VPG

        # create second VMI with same PI, ensure VPG is created
        context = {'operation': 'CREATE',
                   'user_id': '',
                   'is_admin': True,
                   'roles': ''}
        ...
        body = {'context': context, 'data': data}
        resp = self._api_svr_app.post_json('/neutron/port', body)
        port_2_dict = json.loads(resp.text)

        # ensure new VPG created and PIs are attached
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict.get('virtual-port-groups')
        self.assertTrue(len(vpgs) == 2)
        vpg_names = sorted([vpg['fq_name'][-1] for vpg in vpgs])
        self.assertEqual(vpg_names, ['vpg-internal-0', 'vpg-internal-1'])

        # verify PIs are attached to new VMI
        for vpg in vpgs:
            vpg_obj = self._vnc_lib.virtual_port_group_read(id=vpg['uuid'])
            if vpg_obj.fq_name[-1] == 'vpg-internal-1':
                pis = vpg_obj.get_physical_interface_refs()
                self.assertEqual(len(pis), 2)

        # Case 5
        # Delete VMI removes VPG
        # Expectation:
        # API-server will allow deleting first VMI and it also
        # deletes internal VPG-0.

        # delete first port
        self.delete_resource('port', proj_uuid, port_1_dict['id'])

        # ensure first VPG gets deleted
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict['virtual-port-groups']
        self.assertEqual(len(vpgs), 1)
        self.assertEqual(vpgs[0]['fq_name'][-1], 'vpg-internal-1')

        # vpg zk element fq name should get freed from zookeeper
        vpg_zk_pi_1_fq_name = ['default-global-system-config',
                               fabric_name,
                               pi_uuid[0]]
        vpg_zk_pi_1_fq_name_str = ':'.join(vpg_zk_pi_1_fq_name)
        self.assertTrue(mock_zk.get_vpg_from_id(0) is None)
        self.assertEqual(mock_zk.get_vpg_from_id(1), vpg_zk_pi_1_fq_name_str)

        # delete second port
        self.delete_resource('port', proj_uuid, port_2_dict['id'])

        # ensure first VPG gets deleted
        vpg_dict = self._vnc_lib.virtual_port_groups_list()
        vpgs = vpg_dict['virtual-port-groups']
        self.assertEqual(len(vpgs), 0)
        ...
        self._vnc_lib.virtual_network_delete(id=vn_obj.uuid)
        self._vnc_lib.virtual_network_delete(id=vn2_obj.uuid)
    # end test_virtual_port_group_ironic_workflow
    ...
    def test_baremetal_logical_interface_bindings_multiple_bonds(self):
        ...
        def _test_multiple_bonds(tors=None, bonds=None):
            ...
            for tor_name, tor_info in tors.items():
                ...
                tor_info['pr_obj'] = self._vnc_lib.physical_router_read(
                    id=tor_info['pr_uuid'])
            ...
            for bond, bond_info in bonds.items():
                ...
                for t in range(len(bond_info['tors'])):
                    ...
                    if len(switch_interfaces) > 1:
                        ...
                        # Make sure physical interface starting with "ae" got
                        # created
                        phy_interface_found = False
                        pi_dict = self._vnc_lib.physical_interfaces_list()
                        lis = pi_dict['physical-interfaces']
                    ...
                    # Verify that the ESI is set correctly in the physical
                    # interfaces
                    if tors.keys() > 1 and len(switch_interfaces) > 1:
                        ...
                        fq_name = [
                            ...
                            switch_name,
                            switch_intf]
                        pi_uuid = self._vnc_lib.fq_name_to_id(
                            'physical-interface', fq_name)
                        pi_obj = self._vnc_lib.physical_interface_read(
                            id=pi_uuid)
                        if pi_obj.ethernet_segment_identifier:
        ...
        # Build various test topoloties with different set of bond
        # configurations
        tors = {'tor-1': {'interfaces': ['int11',
                                         'int12',
                                         'int13',
                                         'int14'],
                          'pi_uuid': {},
        ...
        b1 = {'bond-1': {'tors': [{'name': 'tor-1',
                                   'interfaces': ['int11',
                                                  'int12']}]},
              'bond-2': {'tors': [{'name': 'tor-1',
                                   'interfaces': ['int13']},
                                  {'name': 'tor-2',
                                   'interfaces': ['int23']}]},
              }
            ...
            if sg['id'] == sg1_dict['id']:
                for rule in sg['security_group_rules']:
                    if rule['direction'] == 'ingress':
                        self.assertEqual(
                            rule['remote_group_id'], sg2_dict['id'])
                found += 1
            if sg['id'] == sg2_dict['id']:
                for rule in sg['security_group_rules']:
                    if rule['direction'] == 'ingress':
                        self.assertEqual(
                            rule['remote_group_id'], sg1_dict['id'])
                found += 1
    ...
    def _create_port_with_sg(self, proj_id, port_security):
        ...
        return self.create_resource(
            'port', proj_id, extra_res_fields={
                'network_id': net_q['id'], 'security_groups': [
                    sg_q['id']], 'port_security_enabled': port_security})
    ...
    def test_allowed_address_with_extra_space(self):
        ...
        self.create_resource(
            ...
            extra_res_fields={
                ...
                'cidr': '10.2.0.0/24',
                'ip_version': 4})
        # adding extra space in IPv6 address is invalid. But for IPv4 address
        # adding extra space is still valid.
        with ExpectedException(webtest.app.AppError):
            self.create_resource(
                'port',
                proj_obj.uuid,
    ...
    def test_update_port_with_port_security_disabled_and_sg(self):
        ...
        with ExpectedException(webtest.app.AppError):
            self.update_resource(
                'port', port_q['id'], proj_obj.uuid, extra_res_fields={
                    'port_security_enabled': False})
    ...
    def test_fixed_ip_conflicts_with_floating_ip(self):
        ...
        net_q = self.create_resource(
            'network', proj_obj.uuid, extra_res_fields={
                'router:external': True, 'port_security_enabled': True})
        ...
        fip_q = self.create_resource(
            'floatingip', proj_obj.uuid, extra_res_fields={
    ...
    def test_empty_floating_ip_body_disassociates(self):
        ...
        pub_net_q = self.create_resource(
            'network', proj_obj.uuid, extra_res_fields={
    ...
    def test_subnet_timestamps(self):
        ...
        proj_id = self._vnc_lib.fq_name_to_id(
            'project', fq_name=[
                'default-domain', 'default-project'])
        sn_dict_2 = self.update_resource(
            'subnet', sn_id, proj_id, extra_res_fields={
                'name': 'test-subnet-timestamps'})
        # verify created timestamp and updated timestamp are not same
    ...
    def test_fip_owner(self):
        admin_proj_obj = vnc_api.Project(
            'admin-proj-%s' %
            (self.id()), vnc_api.Domain())
    ...
    def test_dpdk_compute_neutron_trunk_port_bindings(self):
        vn_obj = vnc_api.VirtualNetwork(self.id())
        vn_obj.add_network_ipam(vnc_api.NetworkIpam(),
                                vnc_api.VnSubnetsType(
                                    [vnc_api.IpamSubnetType(
                                        vnc_api.SubnetType('192.168.50.0',
                                                           24))]))
        self._vnc_lib.virtual_network_create(vn_obj)
        vr_obj = vnc_api.VirtualRouter("my-dpdk-host")
        vr_obj.set_virtual_router_dpdk_enabled(True)
        self._vnc_lib.virtual_router_create(vr_obj)
        ...
        # Update VMI bindings with dpdk host_id
        data2 = {'binding:host_id': 'my-dpdk-host'}
        self.update_resource(
            'port',
            port_dict['id'],
            proj_uuid,
        ...
        port_update_obj = self._vnc_lib.virtual_machine_interface_read(
            id=port_dict['id'])
        port_update_dict = port_update_obj.__dict__

        for key_val in \
            port_update_dict[
                '_virtual_machine_interface_bindings'].get_key_value_pair():
            if key_val.get_key() != 'vif_type' or \
                    key_val.get_key() != 'vif_details':
                continue
            if key_val.get_key() == 'vif_details':
                vif_details = json.loads(key_val.get_value())
                self.assertNotEqual(
                    vif_details['vhostuser_socket'], None)
                self.assertEqual(
                    vif_details['vhostuser_vrouter_plug'], True)
                self.assertEqual(
                    vif_details['vhostuser_mode'], "server")
            if key_val.get_key() == 'vif_type':
                self.assertEqual(
                    key_val.get_value(), 'vhostuser')

    # end test_dpdk_compute_neutron_trunk_port_bindings
...
class TestExtraFieldsPresenceByKnob(test_case.NeutronBackendTestCase):
    ...
    def setUpClass(cls):
        super(
            TestExtraFieldsPresenceByKnob, cls).setUpClass(
            extra_config_knobs=[
                ('NEUTRON', 'contrail_extensions_enabled', True)])
...
class TestExtraFieldsAbsenceByKnob(test_case.NeutronBackendTestCase):
    ...
    def setUpClass(cls):
        super(
            TestExtraFieldsAbsenceByKnob, cls).setUpClass(
            extra_config_knobs=[
                ('NEUTRON', 'contrail_extensions_enabled', False)])
...
class TestListWithFilters(test_case.NeutronBackendTestCase):
    def test_filters_with_id(self):
        ...
        with test_common.patch(
            ...
            data = {
                'filters': {
                    'id': [
                        fip1_obj.uuid, fip2_obj.uuid], 'tenant_id': [
                        proj2_obj.uuid.replace(
                            '-', '')]}}
            body = {'context': context, 'data': data}
            resp = self._api_svr_app.post_json(
                '/neutron/floatingip', body)
    ...
    def test_filters_with_shared_and_router_external(self):
        ...
        subnet1_q = self.create_resource(
            'subnet',
            proj_obj.uuid,
            extra_res_fields={
                'network_id': vn1_obj.uuid,
                'cidr': '10.0.0.0/24',
                'ip_version': 4})
        subnet2_q = self.create_resource(
            'subnet',
            proj_obj.uuid,
            extra_res_fields={
                'network_id': vn2_obj.uuid,
                'cidr': '10.1.0.0/24',
                'ip_version': 4})
        subnet3_q = self.create_resource(
            'subnet',
            proj_obj.uuid,
            extra_res_fields={
                'network_id': vn3_obj.uuid,
                'cidr': '10.2.0.0/24',
                'ip_version': 4})
