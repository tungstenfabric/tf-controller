...
class DataCenterInterconnectServer(ResourceMixin, DataCenterInterconnect):
    ...
    def _validate_dci_lrs_fabrics_based_on_dcitype(cls, db_conn, dci,
        ...
        if dci_type is None and read_dci is not None:
            dci_type = read_dci.get('data_center_interconnect_type')
        if dci_type is None:
            dci_type = 'inter_fabric'

        if dci_type != 'intra_fabric':
            dci_mode = dci.get('data_center_interconnect_mode')
            if dci_mode is None and read_dci is not None:
                dci_mode = read_dci.get('data_center_interconnect_mode')
            if dci_mode is None:
                dci_mode = 'l3'
            if dci_mode == 'l2':
