...
class VirtualNetworkServer(ResourceMixin, VirtualNetwork):
    ...
    def _check_provider_details(cls, obj_dict, db_conn, create):
        ...
        # TODO(dji): result should tell who exist who does not
        if (is_provider_network and not properties) or \
           (not is_provider_network and properties):
           return (False,
                   'is_provider_network and provider_properties '
                   'must exist together or do not exist together')
        ...
        if is_provider_network:
            ...
            if vn.get('is_provider_network', False):
                ...
                if not cls._compare_old_and_new_properties(old_properties, properties):
                    return (False,
            ...
            return cls._check_properties(properties, create)

        if vn.get('is_provider_network', False):
            return (False, 'cannot update provider VN to non-provider VN')

        return (True, '')
    # end _check_provider_details

    @classmethod
    def _check_properties(cls, properties, is_create):
        action = 'update'
        if is_create:
            action = 'create'
        if properties.get('segmentation_id', None) is None:
            return (False, "Segmenation ID must be configured to {0}".format(action))
        if properties.get('physical_network', None) is None:
            return (False, "Physical Network must be configured to {0}".format(action))
        return (True, '')
    # end _check_properties

    @classmethod
    def _compare_old_and_new_properties(cls, old_properties, new_properties):
    ...
    # end _compare_old_and_new_properties
