...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def _check_serviceprovider_fabric(
        ...
        """
        # Check if vlan-id received is valid or VMI update
        if not vlan_id:
            return True, '', None
    ...
    @classmethod
    def _create_fabric_enterprise_vmi_validation_node(
        cls, db_conn, db_vlan_id, vpg_uuid, vn_uuid, vlan_id, vmi_uuid,
        is_untagged_vlan, db_is_untagged_vlan, vmi_lock_args):

        vmi_validation_znode = os.path.join(
            _DEFAULT_ZK_ENTERPRISE_FABRIC_VALIDATION_PATH_PREFIX,
            ('virtual-port-group:%s/vlan:%s')
            % (vpg_uuid, vlan_id))
        try:
            cls.db_conn._zk_db._zk_client.create_node(
            vmi_validation_znode, value=vmi_uuid)
            
            def undo_create_enterprise_vmi_validation_node():
                cls.db_conn._zk_db._zk_client.delete_node(vmi_validation_znode, True)
                return True, '', None
            get_context().push_undo(undo_create_enterprise_vmi_validation_node)
            ...
            if db_vlan_id and (db_vlan_id != vlan_id):
                if not is_untagged_vlan:
                    vmi_validation_znode = os.path.join(
                        ...
                        ('virtual-port-group:%s/vlan:%s')
                        % (vpg_uuid, db_vlan_id))
                    return True, '', {'delete_validation_znode': vmi_validation_znode}
            ...
            if ok:  # Not STALE
                ...
                vlan_validation_znode = os.path.join(
                    _DEFAULT_ZK_ENTERPRISE_FABRIC_VALIDATION_PATH_PREFIX,
                    ('virtual-port-group:%s/virtual-network:%s')
                    % (vpg_uuid, vn_uuid))
                msg = "Znode with %s virtual_machine_interface already exists" % vmi_uuid
                return False, (400, msg), {'delete_validation_znode': vlan_validation_znode}
            else:  # STALE
                cls.db_conn._zk_db._zk_client.update_node(
                vmi_validation_znode, repr(vmi_uuid).encode('ascii'))
    ...
    def _check_enterprise_fabric(
        ...
        :  is_untagged_vlan:
        :    True: untagged
        :    False:         :  db_is_untagged_vlan: old is_untagged_vlan from ZK
        :  db_is_untagged_vlan: old is_untagged_vlan from ZK
        :Returns
        :  (True, '', {delete_validation_znode': db_vlan_id}): PASS condition
        :  (False, (<code>, msg), None): Fail condition
        ...
        if is_untagged_vlan:  # untagged VLAN
            ...
            try:
                ...
                def undo_create_enterprise_validation_node():
                    cls.db_conn._zk_db._zk_client.delete_node(validation_znode, True)
                    return True, "", None
                get_context().push_undo(undo_create_enterprise_validation_node)
            ...
            with ZookeeperLock(**vmi_lock_args):
                try:
                    cls.db_conn._zk_db._zk_client.create_node(
                    vlan_validation_znode, value=vlan_id)
            
                    def undo_create_enterprise_vlan_validation_node():
                        cls.db_conn._zk_db._zk_client.delete_node(vlan_validation_znode, True)
                        return True, '', None
                    get_context().push_undo(undo_create_enterprise_vlan_validation_node)

                    ok, result, del_znode_kwargs = _create_fabric_enterprise_vmi_validation_node(
                        db_conn, db_vlan_id, vpg_uuid, vn_uuid, vlan_id, vmi_uuid,
                        is_untagged_vlan, db_is_untagged_vlan, vmi_lock_args)
                    if not ok:
                        ...
                        if db_is_untagged_vlan is not None:
                            ...
                            if (is_untagged_vlan and not db_is_untagged_vlan):
                                vlan_validation_znode = os.path.join(
                                    ...
                                    'virtual-port-group:%s/virtual-network:%s'
                                    % (vpg_uuid, vn_uuid))
                                return True, '', {'delete_validation_znode': vlan_validation_znode}

                        msg = "Znode with %s vlan_id already exists" % vlan_id
                        return False, (400, msg), None
                    else:  # STALE
                        cls.db_conn._zk_db._zk_client.update_node(
    ...
    def _delete_serviceprovider_validations_znodes(cls, obj_dict, db_conn, vpg_uuid):
        ...
        ok, (vlan_id, is_untagged_vlan, links) = cls.get_vlan_phy_links(None,
            obj_dict)
        if not ok:
