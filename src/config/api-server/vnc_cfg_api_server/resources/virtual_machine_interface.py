...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def _check_serviceprovider_fabric(
        ...
        try:
            ...
            try:
                zk_vmi_uuid = cls.db_conn._zk_db._zk_client.read_node(validation_znode)
                ...
                if is_untagged_vlan:
                     msg = ("Virtual machine interface:(%s) already exists for "\
                            "virtual port group:(%s)" % (zk_vmi_uuid, vpg_uuid))
                else:
                     msg = ("Virtual machine interface:(%s) already exists in "\
                            "virtual network:(%s) with vlan(%s)" %
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        for vpg_back_ref in obj_dict.get('virtual_port_group_back_refs',
            ...
            fabric = read_result
            fabric_enterprise_style = fabric.get(
                'fabric_enterprise_style') or False
