...
class TestDataCenterInterconnect(test_case.ApiServerTestCase):
    ...
    def _verify_l2_dci_create_edit(self, dciname, dict_fabrics, dict_vns,
        ...
        dci_obj.set_virtual_network_list([])
        dci_obj.set_fabric_list(fabric_list)
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
        ...
        fabric_list.append(dict_fabrics[non_dci_fabric_name])
        dci_obj.set_fabric_list(fabric_list)
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                ...
                "with DCI-Gateway rb_roles." % non_dci_fabric_name):
            func_call(dci_obj)

        fabric_list.remove(dict_fabrics[non_dci_fabric_name])
        dci_obj.set_fabric_list(fabric_list)
        vn_list.append(dict_vns[routed_vn_name])
        dci_obj.set_virtual_network_list(vn_list)
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
        ...
        vn_list.remove(dict_vns[routed_vn_name])
        if func_call == self._vnc_lib.data_center_interconnect_create:
            dci_obj.set_fabric_list([])
            dci_obj.set_virtual_network_list([])
            self._add_refs(fabric_list, dci_obj.add_fabric)
            self._add_refs(vn_list, dci_obj.add_virtual_network)
            dci_uuid = func_call(dci_obj)
        else:
            dci_uuid = dci_obj.get_uuid()
