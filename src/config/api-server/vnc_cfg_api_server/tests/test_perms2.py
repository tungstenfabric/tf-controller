...
def token_from_user_info(user_name, tenant_name, domain_name, role_name,
    ...
    token_dict = {
        ...
        'X-Project-Id': tenant_id.replace('-', '') if tenant_id else '',
        'X-Domain-Id' : domain_id.replace('-', '') if domain_id else '',
...
class TestPermissions(test_case.ApiServerTestCase):
    ...
    def test_set_tag(self):
        ...
        for tag_type in ["deployment", "site"]:
            vnc_aal_add_rule(
                self.admin.vnc_lib,
                global_rg,
                "set-tag.%s cloud-admin:C" % tag_type,
            )
        ...
        for tt, tv, allowed in tag_list:
            tag_obj = Tag(tag_type_name=tt, tag_value=tv, parent_obj=self.alice.project_obj)
        ...
        for tt, tv, allowed in tag_list:
            try:
                self.alice.vnc_lib.set_tag(vn_obj, tt, tv)
                tag_set = True
            except PermissionDenied as e:
                tag_set = False
        ...
        for tt, tv, allowed in tag_list:
            try:
                self.alice.vnc_lib.unset_tag(vn_obj, tt)
                tag_unset = True
            except PermissionDenied as e:
                tag_unset = False
        ...
        for tt, tv, allowed in tag_list:
            self.admin.vnc_lib.set_tag(vn_obj, tt, tv)
        vn = self.admin.vnc_lib.virtual_network_read(id=vn_obj.uuid)
        tag_refs = vn.get_tag_refs()
        ...
        for tag_type in ["deployment", "site"]:
            vnc_aal_add_rule(
                self.admin.vnc_lib,
                global_rg,
                "set-tag.%s alice-role:C, cloud-admin:C" % tag_type,
            )
        for tt, tv, allowed in tag_list:
            if not allowed:
                self.alice.vnc_lib.unset_tag(vn_obj, tt)
    ...
    def test_bug_1646200(self):
        ...
        (ok, cassandra_result) = self._api_server._db_conn._object_db.object_update(
	...
	# Create  logical router as user alice
        ...
        try:
            ...
            self.assertTrue(True, 'Unable to update LR external GW ... test passed')
