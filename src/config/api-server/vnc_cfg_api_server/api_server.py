...
class ConfigVar(object):
    ...
    def is_value_correct_type_and_expected(self):
        if type(self._var_value) != UPDATE_CONFIG_ALLOWED_VARS[self._var_name]:
        ...
        if (self._var_name == 'log_level'
            ...
            msg = (('%s Incorrect log level. Possible values are %s') %
                    (self._var_value, ",".join(self.LOG_LEVEL_LIST)))
    ...
    def set_var(self):
        if self._var_value:
            self._var_value = (self.BOOL_STR_MAP[self._var_value]
                               if self._var_value in self.BOOL_STR_MAP.keys()
                               else str(self._var_value))
            self.is_var_allowed()
...
class VncApiServer(object):
    ...
    def update_configs_http_post(self):
        ...
        return {'msg': end_log_msg, 'result': 202}
    ...
    def configs_http_get(self):
        ...
        get_payload = {}
        for config_var_name in UPDATE_CONFIG_ALLOWED_VARS.keys():
            config_var_obj = ConfigVar(self, config_var_name)
