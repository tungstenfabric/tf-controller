...
class TestDCI(TestAnsibleCommonDM):
    ...
    def test_dci_ibgp(self):
        ...
        lr2_refs = lr2.get_virtual_network_refs()
        vn2_uuid = self.get_val(lr2_refs[0], 'uuid')
        vn2 = self._vnc_lib.virtual_network_read(id=vn2_uuid)
        vn2_refs = vn2.get_routing_instances()
        ri2_uuid = self.get_val(vn2_refs[0], 'uuid')
        ri2_obj = self._vnc_lib.routing_instance_read(id=ri2_uuid)
        ri2_refs = ri2_obj.get_route_target_refs()
        ri.append(self.get_val(ri2_refs[0], 'to')[0])
        ...
        self.validate_abstract_configs_ibgp(pr1, pr2, ri)
    ...
    def test_dci_ebgp(self):
        ...
        self.validate_abstract_configs_ebgp(pr1, pr2, ri)
    ...
    def validate_abstract_configs_ebgp(self, pr1, pr2, ri):
        ...
        asn_config1 = dac1.get('routing_instances', {})
        ...
        asn_config2 = dac2.get('routing_instances', {})
    ...
    @retries(5, hook=retry_exc_handler)
    def validate_abstract_configs_ibgp(self, pr1, pr2, ri):
        ac1 = FakeJobHandler.get_dev_job_input(pr1.name)
        dac1 = ac1.get('device_abstract_config')
        bgp_config1 = dac1.get('features', {}).get(
            'overlay-bgp', {}).get('bgp', [])
        ...
        self.assertEqual(bgp_peer1[0].get('ip_address'), '30.30.0.23')
        self.assertEqual(asn_config1[0].get('import_targets'), ri)
        ...
        self.assertEqual(bgp_peer2[0].get('ip_address'), '30.30.0.22')
        self.assertEqual(asn_config2[0].get('import_targets'), ri)
