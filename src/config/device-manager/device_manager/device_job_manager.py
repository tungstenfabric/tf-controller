        ...
        for device_id in device_list:
            if not is_delete:
                ...
                device_product_name = result.get(
                    "physical_router_product_name")
                if not result.get("physical_router_junos_service_ports"):
                    device_service_ports = [22]
                else:
                    device_service_ports = result.get(
                        "physical_router_junos_service_ports").get(
                            "service_port")
            ...
            if device_service_ports:
                device_json.update({"device_service_ports":
