...
class DataCenterInterconnectDM(DBBaseDM):
    ...
    def get_dci_import_policies(self, pr_obj=None, lr_obj=None):
        ...
        if lr_obj is not None:
            import_policies.add(DMUtils.get_dci_policy_name(lr_obj))
        ...
        for vn_uuid in self.virtual_networks:
            ...
            if not vn:
                continue
            _, imports = vn.get_route_targets()
            if imports and len(imports) > 0:
                policy_name = DMUtils.get_dci_policy_name(vn)
    ...
    @classmethod
    def update_peer_dict(cls, neigh_bgpr_dict, in_bgpr_list, in_dci_name):
        for peer_bgpr in in_bgpr_list:
            if peer_bgpr not in neigh_bgpr_dict:
                neigh_bgpr_dict[peer_bgpr] = set()
            neigh_bgpr_dict[peer_bgpr].add(in_dci_name)
    ...
    def get_dci_peers(cls, pr_obj):
        ...
        and value is the list of dci names. each dci_name has pr_obj
        in one of lr list for l3 dci type else in fabric list for l2 dci
        type,  along with this peer pr key exists in this dci for other
        fabric.
                ...
                for f_uuid in dci.fabrics:
                    ...
                    for pr_uuid in fabric_obj.physical_routers:
                        ...
                        if pr.has_rb_role("DCI-Gateway") and pr.bgp_router:
                            neigh_bgpr_list.add(pr.bgp_router)
                if l2_dci_include == True and len(neigh_bgpr_list) > 0:
                    cls.update_peer_dict(
                        neigh_bgpr_dict, neigh_bgpr_list, dci.name)
