...
class PhysicalInterfaceDM(DBBaseDM):
    ...
    def delete_obj(self):
        self.delete_job_trans(old_pr_list=[self.physical_router])
...
class LogicalInterfaceDM(DBBaseDM):
    ...
    def delete_obj(self):
        if self.physical_interface:
            ...
            self.delete_job_trans(old_pi_list=[self.physical_interface])
...
class VirtualNetworkDM(DBBaseDM):
    ...
    def get_prefixes(self, pr_uuid=None, only_routedvn_prefix=False):
        lr = None
        prefix_set = set()
    ...
    def get_dci_peer_fabric_rt(self, pr_obj):
        """Get all route target (RT) of peer fabric of all L2 DCI.

        It walks through vn_obj all L2 DCI mode objects and retrieves
        Peer Fabric's Route Targets (RT) to import as import_target set. This
        RT is type 1 RT of fabric needed to import for L2 DCI OTT feature.
        : Args:
        : pr_obj: physical router object whose peer fabric RT needed
        : return:
        ...
        for dci_uuid in self.data_center_interconnects:
            dci = DataCenterInterconnectDM.get(dci_uuid)
            if not dci or dci.is_this_inter_fabric() is False or \
                    dci.dci_mode == 'l3':
                continue
            for f_uuid in dci.fabrics:
                if f_uuid == pr_obj.fabric:
                    continue
                ...
                for pr_uuid in fabric_obj.physical_routers:
                    ...
                    if not pr.bgp_router or not pr.has_rb_role("DCI-Gateway"):
                        continue
                    bgp = BgpRouterDM.get(pr.bgp_router)
                    if bgp and bgp.params and bgp.params.get('address'):
                        fabric_rt = bgp.params.get('local_autonomous_system')\
                            or bgp.params.get('autonomous_system')
                        if fabric_rt:
                            import_rts.add(fabric_rt)
        for rt in import_rts:
            if int(rt) <= 65535:
                postfix = ':7999999'
            else:
                postfix = 'L:7999'
            import_targets.add('target:%s%s' % (rt, postfix))
        return import_targets
    # end get_dci_peer_fabric_rt
...
class DataCenterInterconnectDM(DBBaseDM):
    ...
    def get_dci_peers(cls, pr_obj):
        ...
        for dci in list(cls._dict.values()) or []:
            ...
            if dci_include is True and len(neigh_bgpr_fabric) > 0:
    ...
    def get_client_rp_name_for_dci(cls, rpname, rpobj, dci_obj=None):
        ...
        for o_term in rpobj.routing_policy_entries or []:
            ...
            if o_action and o_action == 'accept':
                return DMUtils.get_dci_client_rp_name(dci_obj, rpname)
        return rpname
...
class DMCassandraDB(VncObjectDBClient):
    ...
    _PR_ASN_CF = ConfigKeyspaceMap.get_cf_name(_KEYSPACE,'PR_VN_IP_CF')
    _NI_IPV6_LL_CF = ConfigKeyspaceMap.get_cf_name(_KEYSPACE,'PR_ASN_CF')
    # PNF table
