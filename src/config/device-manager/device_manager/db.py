...
class DataCenterInterconnectDM(DBBaseDM):
    ...
    @classmethod
    def update_peer_dict(cls, neigh_bgpr_dict, in_bgpr_list,
                         in_dci_name, peer_fabrics):
        for peer_bgpr in in_bgpr_list:
            ...
            neigh_bgpr_dict[peer_bgpr]['fabrics'].update(peer_fabrics)
                        ...
                        if pr.has_rb_role("DCI-Gateway"):
                            if (pr_uuid == lpr_uuid):
                                ...
                                if fabric_obj and fabric_obj.name:
                                    peer_fabrics.add(fabric_obj.name)
                ...
                for f_uuid in dci.fabrics:
                    ...
                    for pr_uuid in fabric_obj.physical_routers:
                        ...
                        if pr.has_rb_role("DCI-Gateway") and pr.bgp_router:
                            found_peer_pr = True
                            neigh_bgpr_list.add(pr.bgp_router)
                    if found_peer_pr is True:
                        peer_fabrics.add(fabric_obj.name)

            if dci_include == True and len(neigh_bgpr_list) > 0:
                cls.update_peer_dict(
                    neigh_bgpr_dict, neigh_bgpr_list, dci.name, peer_fabrics)
