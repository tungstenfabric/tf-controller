    ...
    def _set_vn_vlanid(self, vlan, vn):
        for vmi_uuid in vn.virtual_machine_interfaces or []:
            ...
            if int(vlan_tag) > 0:
                ...
                # for enterprise style only one vlan id valid per VN.
                # for SP style, multiple vlanid allowed per VPG for same VN
                # though abstract config has single vlan id, so if junos
                # allows change abstract config's this vlan_id as a list,
                # until junos team confirms, lets break here on first vlanid
                break
    # end _set_vn_vlanid
