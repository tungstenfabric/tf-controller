...
# TODO(dji): does vnc calls in ST class triger event handling?
class VirtualPortGroupST(ResourceBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        self.virtual_machine_interface_uuid = None # uuid of placeholder VMI
        self.vlan_id = None
    # end __init__
    ...
    def validate_physical_interfaces(self):
        for pi_uuid, vmi_st_names in self.pi_uuid_to_vmi_st_names.items():
            for vmi_st_name in vmi_st_names:
                vmi_st = ResourceBaseST.get_obj_type_map().get('virtual_machine_interface').get(vmi_st_name)
    ...
    def validate_virtual_port_group(self):
        if len(self.pi_uuid_to_vmi_st_names) == 0:
            try:
                self._vnc_lib.virtual_machine_interface_delete(id=self.virtual_machine_interface)
                self._vnc_lib.virtual_port_group_delete(id=self.uuid)
            except NoIdError:
                pass
            VirtualPortGroupST.delete(self.name)
        return
    # end validate_virtual_port_group

    def update_virtual_machine_interface_uuid(self, virtual_machine_interface_uuid):
    ...
    def update_pi_uuid_to_vmi_st_name(self, pi_uuid, vmi_st_name):
        self.pi_uuid_to_vmi_st_names[pi_uuid] = self.pi_uuid_to_vmi_st_names.get(pi_uuid, []) + [vmi_st_name]
        return
