...
class VirtualPortGroupST(ResourceBaseST):
    ...
    def update_physical_interfaces(self):
        self._logger.notice("Starts upadting physical interfaces")
        new_pis = set()
        ...
        self._logger.notice("Finished upadting physical interfaces")
    # end update_physical_interfaces

    def validate_virtual_port_group(self):
        self._logger.notice("Starts validating virtual port group")
        if len(self.physical_interface_uuids) == 0 and \
           not self.virtual_port_group_user_created:
            try:
