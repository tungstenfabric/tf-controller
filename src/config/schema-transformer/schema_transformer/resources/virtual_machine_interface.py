...
class VirtualMachineInterfaceST(ResourceBaseST):
    ...
    prop_fields = ['annotations', 'virtual_machine_interface_bindings',
    ...
    def evaluate(self, **kwargs):
        ...
        self.process_vpg()
        self.validate_vpg_related_info()
    # end evaluate

    def validate_vpg_related_info(self):
        if self.virtual_machine is None:
            self.delete_physical_interface_uuid_annotation()
    # end validate_vpg_related_info

    def process_vpg(self):
        self._logger.notice("Process VPG collecting information"
                            " of vlan_id and physical interface")
        vlan_id, pi, fabric = \
            self.collect_vlan_id_and_physical_interface_and_fabric()
        self._logger.notice("Process VPG collected vlan_id"
                            " and physical interface and fabric")
        if vlan_id is None or pi is None or fabric is None:
            return
        self.update_physical_interface_uuid_annotation(pi.get_uuid())
        self._logger.notice("Process VPG checking whether desired VPG exists")
        vpg = self.find_vpg(pi, fabric)
        if vpg is None:
            self._logger.notice("Process VPG - "
            ...
            self._logger.notice("Process VPG - "
                                "VPG update finished")
        self._logger.notice("Process VPG finished processing VPG")
    # end process_vpg

    def update_physical_interface_uuid_annotation(self, pi_uuid):
        self.obj.add_annotations(KeyValuePair(key='pi-uuid', value=pi_uuid))
        try:
            self._vnc_lib.virtual_machine_update(self.obj)
        except Exception as e:
            self._logger.error("Unexpected exception "
                               "during updating annotation: %s"
                               % e.msg)
    # end update_physical_interface_uuid_annotation

    def delete_physical_interface_uuid_annotation(self):
        self.obj.del_annotations(elem_position='pi-uuid')
        try:
            self._vnc_lib.virtual_machine_update(self.obj)
        except Exception as e:
            self._logger.error("Unexpected exception "
    ...
    def collect_vlan_id_and_physical_interface_and_fabric(self):
        ...
        self._logger.notice("Collected hostname: %s" % hostname)
        if hostname == "null":
            return (None, None, None)
        # get physnet name and vlan_id
        physnet, vlan_id = self.get_physnet_and_vlan_id()
        self._logger.notice("Collected physnet: %s, "
        ...
        self._logger.notice("Collected switch_id: %s, "
        ...
        if physical_interface is None:
            self._logger.notice("Physical Interface not found")
        else:
            self._logger.notice("Collected physical_interface: %s"
                                % physical_interface.get_fq_name_str())
        if fabric is None:
            self._logger.notice("Physical Router parent Fabric not found")
        else:
            self._logger.notice("Collected Physical Router parent Fabric: %s"
    ...
    def get_hostname(self):
        ...
        if self.virtual_machine_interface_bindings.get('vnic_type', "") \
           != "direct":
            return "null"
        return self.virtual_machine_interface_bindings.get('host_id', "null")
    # end get_hostname
    ...
    def create_vpg_object(self, vlan_id, pi, fabric):
        if vlan_id is None or pi is None or fabric is None:
            self._logger.error("pre-requisites error "
        ...
        try:
            ...
            self._logger.error("Unexcepted error when updating "
                               "virtual port group's "
                               "physical interface reference: %s" % e.msg)
    ...
    def update_vpg_object(self, vpg, vlan_id, pi, fabric):
        ...
        try:
            ...
            self._logger.error("Unexcepted error when updating "
                               "virtual port group's "
                               "fabric virtual machine interface: %s" % e.msg)
    # end update_vpg_object

    def prepare_fabric_vmi(self, vlan_id):
        if self.virtual_network is None:
            self._logger.error("VMI %s does not have a VN" % self.name)
            return None
        parent_obj = \
        ...
        try:
            ...
            self._logger.error("Unexpected exception when "
                               "reading fabric vmi %s: %s"
                               % (vmi_fq_name_str, e.msg))
        return vmi
