...
class VirtualMachineInterfaceST(ResourceBaseST):
    ...
    def set_bindings(self):
        self.virtual_machine_interface_bindings = dict()
        bindings = self.obj.get_virtual_machine_interface_bindings()
        if not bindings:
            return
        for kvp in bindings.get_key_value_pair():
            self.virtual_machine_interface_bindings[kvp.get_key()] =\
                kvp.get_value()
        return
    # end set_bindings
