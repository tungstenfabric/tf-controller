...
class VirtualMachineInterfaceST(ResourceBaseST):
    ...
    def update(self, obj=None):
        ...
        if self.virtual_machine_interface_bindings is not None:
            if self.virtual_machine_interface_bindings.get('is_placeholder', "False") == "True":
                self.virtual_network = None
        return changed
    # end update
    ...
    def process_vpg(self):
        if self.virtual_machine_interface_bindings \
               .get('is_placeholder', "False") == "True":
            return
    ...
    def get_uuids(self, items):
        ...
        if isinstance(items, dict) and len(items.keys()) > 0:
            return [item['uuid'] for item in items.get(items.keys()[0], [])]
    # end get_uuids
    ...
    def create_vpg_object(self, vlan_id, physical_interface):
        ...
        vpg = VirtualPortGroup(
            ...
            parent_obj=parent_project)
        # VPG refers to placeholder VMI
        vmi = VirtualMachineInterface(
            name="vmi-to-vn" + virtual_network.get_uuid(),
            parent_obj=parent_project)
        # TODO(dji): will cause vmi to be evaluated because it has a vn now, fixed by adding is_placeholder in bindings, need test
        vmi.set_virtual_network(virtual_network)
        # TODO(dji): can device manager consume string vlan_id
        vmi.set_virtual_machine_interface_bindings(
            KeyValuePairs(
                key_value_pair=[
                    KeyValuePair(
                        key='vlan_id', value=str(vlan_id)),
                    KeyValuePair(
                        key='is_placeholder', value=str(True))]))
        vmi_uuid = self._vnc_lib.virtual_machine_interface_create(vmi)
        # TODO(dji): need to create vpg first, then update with ref to PI, this cause vpg evaluation problem, fixed by adding is_initialized field, need test
        ...
        count = 0
        # TODO(dji): concurrency issue, need to wait for VPGST to exist, do we need db read instead
        ...
        virtual_port_group_st.update_is_initiated(True)
