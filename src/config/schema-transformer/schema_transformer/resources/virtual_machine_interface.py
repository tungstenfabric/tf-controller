...
class VirtualMachineInterfaceST(ResourceBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        # a first created VMIST won't have one,
        # but during update, it will have one
    ...
    def process_vpg(self):
        ...
        if vlan_id is None or pi is None or fabric is None:
            return
        self.physical_interface_uuid = pi.get_uuid()
        self._logger.notice("Process VPG checking whether desired VPG exists")
        if not self.vpg_exists(pi, fabric):
    ...
    def vpg_exists(self, pi, fabric):
        if pi is None or fabric is None:
            return False
        vpg_fq_name_str = fabric.get_parent_fq_name_str() + \
            ":" + "__sriov_vgp_pi_" + pi.get_uuid() + "__"
        try:
            self._vnc_lib.virtual_port_group_read(fq_name_str=vpg_fq_name_str)
        except NoIdError:
            return False
        else:
            return True
    ...
    def create_vpg_object(self, vlan_id, pi, fabric):
        ...
        # Used exisiting fabric vmi (placeholder vmi) if it exists
        # TODO(dji): should we use exisitng fabric vmi?
        vmi_name = "__sriov_vmi_vn_" + virtual_network.get_uuid() + "__"
        ...
        vpg = VirtualPortGroup(
            name="__sriov_vgp_pi_" + pi.get_uuid() + "__",
            parent_obj=fabric)
        vpg.set_virtual_port_group_user_created(False)
        vpg.set_virtual_machine_interface(vmi)
        # TODO(dji): consider the case the vpg is created
        # after vpg existence check
