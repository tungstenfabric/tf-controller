...
from vnc_api.gen.resource_client import RouteTarget, RoutingInstance
from vnc_api.gen.resource_xsd import ConnectionType
from vnc_api.gen.resource_xsd import InstanceTargetType, ServiceChainInfo
from vnc_api.gen.resource_xsd import StaticRouteEntriesType
...
class RoutingInstanceST(ResourceBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        for ri_ref in self.obj.get_routing_instance_refs() or []:
            ...
            if conn_fq_name != 'ERROR':
                ...
                try:
                    self._vnc_lib.ref_update('routing-instance',
                                             self.obj.uuid,
                                             'routing-instance',
                                             ri_ref['uuid'],
                                             None,
    ...
    def update_routing_policy_and_aggregates(self):
        ...
        sc = ResourceBaseST.get_obj_type_map().get(
            'service_chain').get(self.service_chain)
        if sc is None:
            return
        for si_name in sc.service_list:
            ...
            if sc.left_vn == self.virtual_network:
                ...
                ra_set = set(ra for ra, if_type in
                             list(si.route_aggregates.items())
                             if if_type == 'left')
            ...
            for ra_name in self.route_aggregates - ra_set:
                ra = ResourceBaseST.get_obj_type_map().get(
                    'route_aggregate').get(ra_name)
                if ra:
    ...
    def import_default_ri_route_target_to_service_ri(self):
        ...
        if sc is None or not sc.created:
            return update_ri
        left_vn = ResourceBaseST.get_obj_type_map().get(
            'virtual_network').get(sc.left_vn)
    ...
    def locate_route_target(self):
        ...
        if self.is_default:
            inst_tgt_data = InstanceTargetType()
                ...
                'virtual_network')._ri_needs_external_rt(self.virtual_network,
                                                         self.name):
    ...
    def update_static_routes(self):
        ...
        si_set = ResourceBaseST.get_obj_type_map().get(
            'service_instance').get_vn_si_mapping(self.virtual_network)
        for si in si_set or []:
            mode = si.get_service_mode()
            if mode is None or mode != 'in-network-nat':
                self._logger.debug("service mode for %s-%s, skip" %
                                   (si.name, mode))
                continue
            route_tables = ResourceBaseST.get_obj_type_map().get(
                'route_table').get_by_service_instance(si.name)
            ...
            for route_table_name in route_tables:
                route_table = ResourceBaseST.get_obj_type_map().get(
                    'route_table').get(route_table_name)
                ...
                for lr_name in route_table.logical_routers:
                    lr = ResourceBaseST.get_obj_type_map().get(
                        'logical_router').get(lr_name)
                    if lr is None:
                        self._logger.debug("lr is None for: " + lr_name)
                        continue
                    route_targets = route_targets | lr.rt_list | \
                        set([lr.route_target])
                if not route_targets:
                    self._logger.debug("route targets None for: " +
                                       route_table_name)
                    continue
    ...
    def delete_obj(self):
            ...
            if rp:
                ...
                try:
                    rp_obj = ResourceBaseST.get_obj_type_map().get(
                        'routing_policy').read_vnc_obj(fq_name=rp_name)
                    if rp_obj:
        ...
        for ra_name in self.route_aggregates:
            ...
            if ra:
                ...
                try:
                    ra_obj = ResourceBaseST.get_obj_type_map().get(
                        'route_aggregate').read_vnc_obj(fq_name=ra_name)
                    if ra_obj:
        ...
        bgpaas_server = ResourceBaseST.get_obj_type_map().get(
            'bgp_router').get(bgpaas_server_name)
        if bgpaas_server:
            try:
        ...
        for rtgt in rtgt_list or []:
            try:
                ResourceBaseST.get_obj_type_map().get(
                    'route_target').delete_vnc_obj(rtgt['to'][0])
            except RefsExistError:
