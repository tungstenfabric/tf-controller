    ...
    def check_ri_target(self, fq_name, rt_target=None):
        ...
        for rt_ref in rt_refs:
            if rt_ref['to'][0] == rt_target:
                return rt_target
            if int(rt_target.rsplit(':', 1)[1]) > 0xFFFF:
                if rt_target in rt_ref['to'][0]:
                    return rt_target
...
class TestBgp(STTestCase, VerifyBgp):
    ...
    def _create_router(self, name, mgmt_ip, vendor='juniper', product='mx',
    ...
    def _delete_routers(self, bgp_router=None, pr=None):
        if pr:
    ...
    def _delete_fabric(self, fab=None):
        if fab:
    ...
    def test_ibgp_auto_mesh_fabric_phy_rtr_to_bgp_rtr_dependency(self):
        ...
        self._delete_routers(bgp_router1_fab1, pr1_fab1)
        self._delete_routers(bgp_router2_fab1, pr2_fab1)
        self._delete_routers(bgp_router1_fab2, pr1_fab2)
        self._delete_routers(bgp_router2_fab2, pr2_fab2)
        self._delete_fabric(fab1)
        self._delete_fabric(fab2)
    ...
    def test_ibgp_auto_mesh_fabric_with_different_asn(self):
        ...
        bgp_router2_fab1, pr2_fab1 = self._create_router(
            'router2_fab1' + self.id(), '1.1.1.2',
            product="qfx1000", role="leaf", fabric=fab1,
            asn=64000)

        bgp_router3_fab1, pr3_fab1 = self._create_router(
        ...
        bgp_router2_fab2, pr2_fab2 = self._create_router(
            'router2_fab2' + self.id(), '2.1.1.2',
            product="qfx1000", role="leaf", fabric=fab2,
            asn=64001)

        bgp_router3_fab2, pr3_fab2 = self._create_router(
        ...
        self._delete_routers(bgp_router1_fab1, pr1_fab1)
        self._delete_routers(bgp_router2_fab1, pr2_fab1)
        self._delete_routers(bgp_router3_fab1, pr3_fab1)
        self._delete_routers(bgp_router1_fab2, pr1_fab2)
        self._delete_routers(bgp_router2_fab2, pr2_fab2)
        self._delete_routers(bgp_router3_fab2, pr3_fab2)
        self._delete_fabric(fab1)
        self._delete_fabric(fab2)
    ...
    @skip('CEM-7483 Skip these until we figure out the reason for flakiness')
    ...
    def test_asn(self):
        ...
        # create virtual machine interface
        vmi_name = self.id() + 'vmi1'
        vmi_fqname = self.project.fq_name + [vmi_name]
