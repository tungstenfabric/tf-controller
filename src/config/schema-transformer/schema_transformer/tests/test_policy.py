...
class TestPolicy(STTestCase, VerifyPolicy):
    ...
    def test_provider_network(self):
        ...
        # fail if provider_details not provided
        vn5_obj1.set_is_provider_network(True)
        self.assertRaises(BadRequest,
                          self._vnc_lib.virtual_network_create,
                          vn5_obj1)

        # set provider_details and try to create
        vn5_obj1.set_provider_properties(
            ProviderDetails(
        ...
        # check is_provider_network of non provider_vn
        # can not be set to True if provider_details not provided
        vn6_obj1.set_is_provider_network(True)
        self.assertRaises(BadRequest,
                          self._vnc_lib.virtual_network_update,
                          vn6_obj1)

        # set providre_details
        vn6_obj1.set_provider_properties(
            ProviderDetails(
                params_dict={"segmentation_id": 100,
                             "physical_network": "physnet1"}))
        self._vnc_lib.virtual_network_update(vn6_obj1)
