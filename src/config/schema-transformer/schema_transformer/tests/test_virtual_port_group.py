...
class VerifyVPG(VerifyCommon):
    @retries(5)
    def check_vpg_exists(self, vlan_id, physical_interface_display_name):
        vpg_dicts = self.api.virtual_port_groups_list()
        vpg_uuids = [vpg_dict['uuid'] for
                     vpg_dict in
                     vpg_dicts['virtual-port-groups']]
        for vpg_uuid in vpg_uuids:
            vpg = self.api.virtual_port_group_read(vpg_uuid)
        ...
        raise Exception(
            ...
            "and physical_interface display name %s not found"
            % (vlan_id, physical_interface_display_name))
    # end check_vpg_exists

    @retries(5)
    def check_vpg_not_exist(self, vlan_id, physical_interface_display_name):
        vpg_dicts = self.api.virtual_port_groups_list()
        vpg_uuids = [vpg_dict['uuid'] for
                     vpg_dict in
                     vpg_dicts['virtual-port-groups']]
                        ...
                        if kvp.key == 'vlan_id' and kvp.value == vlan_id:
                            for pi, _ in vpg.get_physical_interface_refs():
                                if pi.get_display_name() == \
                                    ...
                                    raise Exception(
                                        "virtual_port_group with vlan_id %s, "
                                        "and physical_interface display name "
                                        "%s found"
                                        % (vlan_id,
                                            physical_interface_display_name))
        return
...
class TestVPG(STTestCase, VerifyVPG):
    ...
    def _pre_create_node(self):
        ...
        node_red = Node(
            name='test-node-red',
            node_hostname=self.node_red_hostname)
        ...
        node_green = Node(
            name='test-node-green', node_hostname=self.node_green_hostname)
    ...
    def test_create_vmi(self):
        vmi = VirtualMachineInterface(
            name='test-tvirtual-machine-interface',
            parent_obj=self.project)
        ...
        self._vnc_lib.virtual_machine_interface_delete(id=vmi_uuid)
    # end test_create_vmi
