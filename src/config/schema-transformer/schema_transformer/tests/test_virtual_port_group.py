...
class VerifyVPG(VerifyCommon):
    @retries(5)
    def check_vpg_exists(self, vlan_id, physical_interface_port_id):
        ...
        raise Exception(
            ...
            and physical_interface id %s not found'
            % (vlan_id, physical_interface_port_id))
    # end check_vpg_exists

    @retries(5)
    def check_vpg_not_exist(self, vlan_id, physical_interface_port_id):
        vpg_dicts = self.api.virtual_port_groups_list()
        vpg_uuids = [vpg_dict['uuid'] for
                     vpg_dict in
                     vpg_dicts['virtual-port-groups']]
                        ...
                        if kvp.key == 'vlan_id' and kvp.value == vlan_id:
                            for pi, _ in vpg.get_physical_interface_refs():
                                if pi.get_display_name() == \
                                    ...
                                    raise Exception(
                                        'virtual_port_group with vlan_id %s, \
                                        and physical_interface id %s found'
                                        % (vlan_id,
                                            physical_interface_port_id))
        return
