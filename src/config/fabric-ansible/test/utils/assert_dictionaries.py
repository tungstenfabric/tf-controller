def check_subset_dictionaries(expected, actual):
...
def assert_subset_dictionaries(expected, actual):
    if check_subset_dictionaries(expected, actual):
        return None
    raise AssertionError(str(actual) + " is not " + str(expected))
...
def _compare_values(expected, actual):
    ...
    if isinstance(expected, dict):
        if not isinstance(actual, dict):
            return False
        return check_subset_dictionaries(expected, actual)
