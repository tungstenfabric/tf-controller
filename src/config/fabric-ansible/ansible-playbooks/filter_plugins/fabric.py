...
class FilterModule(object):
    ...
    def _validate_device_to_ztp(self, vnc_api, fabric_info):
        ...
        phy_routers = vnc_api.physical_routers_list(fields=filters).\
...
<<<<<<< HEAD   (b2acc6 Merge "UT for no hbf flag on vhost by default")
...
=======
                create_ipam_subnet_new = IpamSubnets([IpamSubnetType(
                    subnet=FilterModule._new_subnet(sn.get('cidr')),
                    default_gateway=sn.get('gateway'),
                    subnet_uuid=str(uuid.uuid1())
                ) for sn in subnets if int(sn.get('cidr').split('/')[-1]) < 31
>>>>>>> CHANGE (08b034 [DM] Exclude adding existing subnets twice when appending ne)
                ])
