...
class FilterModule(object):
    ...
    def get_node_type(sriov_tlv):
        ...
        If specified sriov_tlv is None, then the node won't be recognized as
        neither OVS nor SRIOV compute.
    ...
    def get_dev_neighbors_details(device_command_output):
        ...
        if (not lldp_neighbors_information or
                not isinstance(lldp_neighbors_information, collections.Mapping)):  # noqa: E501
            return []
    ...
    def decode_tlv_info(custom_tlv):
        ...
        if len(info_value) % 2 != 0:
            raise ValueError("Couldn't decode Custom TLV's info field")
    ...
    def extract_management_ifaces_names(management_tlv):
        ...
        if len(iface_names) == 1 and iface_names[0].lower() == TLV_INFO_NOT_SET:
            return []
    ...
    def create_os_node(self,
        ...
        nodes = []
        ip_to_hostname = self.get_mapping_ip_to_hostname(vnc_api, fabric_fq_name)  # noqa: E501
