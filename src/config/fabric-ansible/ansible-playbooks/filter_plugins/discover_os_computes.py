...
from contrail_command import CreateCCResource  # noqa: E402
from import_server import FilterModule as FilterModuleImportServer  # noqa: E402
...
class FilterModule(object):
    ...
    def import_nodes_to_contrail(all_nodes, cc_client):
        ...
        :return: None
        """
        logging.info("Begin adding nodes {} to Contrail Command"
                     .format(str(all_nodes)))
    ...
    def create_os_node(self, vnc_api, devices_command_output, fabric_uuid,
                       cc_client):
        """
    ...
    def create_os_node_filter(self, job_ctx, devices_command_outputs):
        ...
        try:
            ...
            cc_client = CreateCCResource(
                cc_host, cluster_id, cluster_token, cc_username, cc_password
            )
            os_compute_nodes = self.create_os_node(
                vnc_api, devices_command_outputs, fabric_uuid, cc_client
            )
        except Exception as e:
            errmsg = "Unexpected error: %s\n%s" % (
                str(e), traceback.format_exc()
