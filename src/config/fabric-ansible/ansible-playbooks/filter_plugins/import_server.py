...
from contrail_command import CreateCCResource, CreateCCNodeProfile  # noqa
import jsonschema  # noqa: E402
from job_manager.job_utils import JobVncApi  # noqa: E402
...
class FilterModule(object):
    ...
    def get_cc_port_payload(self, port_dict, local_link_dict):
        cc_port = {"kind": "port",
            "data": {
                ...
                "tag_refs": port_dict.get('tag_refs', []),
                "bms_port_info": {
    ...
    def convert_port_format(
        self, port_list, parent_uuid, parent_name, existing_ports
    ):
        cc_port_list = []

        for port in port_list:
            port['node_uuid'] = parent_uuid
            mac_addr = port['mac_address']

            if not port.get('name'):
                port['name'] = "p-" + mac_addr.replace(":", "")[6:]

            port['fq_name'] = [
                'default-global-system-config', parent_name, port['name']
            ]

            existing_port_uuid = self._get_existing_port_uuid(
                existing_ports, port['fq_name'], mac_addr
            )

            if existing_port_uuid is not None:
                port['uuid'] = existing_port_uuid

            port = self.translate(port)

            if 'tags' in port and len(port['tags']) > 0:
                port['tag_refs'] = self._tag_values_to_tag_refs(port['tags'])

            local_link_dict = {k: v for k, v in list(port.items()) if k in
                               ['port_id', 'switch_info', 'switch_id']}

            self._logger.warning("dvs-name:: " + str(port.get('dvs_name')))
            cc_port = self.get_cc_port_payload(port, local_link_dict)
            cc_port_list.append(cc_port)
        return cc_port_list

    def _get_existing_port_uuid(self, ports, fq_name, mac_address):
    ...
    def _tag_values_to_tag_refs(self, tag_values):
        refs = []
        for v in tag_values:
            refs.append({"to": ["label={}".format(v)]})
        return refs

    def _unwrap_list_of_nested_resources(self, resources):
        """
        Removes extra key with type name from list of resources.
        ...
        the function above will give the following output:
        [
            {
                uuid: ...,
                name: ...,
        ...
        for i in range(len(resources)):
            keys = list(resources[i])
            if len(keys) != 1:
                self._logger.error(
                    "Expected nested resource. Got: {}.".format(
                        pprint.pformat(resources[i])
                    )
                )

            data = resources[i][keys[0]]
            resources[i] = data


    def import_nodes(self, data, cc_client):
        if not (isinstance(data, collections.Mapping) and "nodes" in data):
            return []
        ...
        self._unwrap_list_of_nested_resources(existing_nodes)
        self._unwrap_list_of_nested_resources(existing_ports)
        self._unwrap_list_of_nested_resources(existing_tags)

        for node in nodes:
            payload, response = self._create_cc_node(node, existing_nodes, cc_client)
            if payload is None or len(response) == 0:
            ...
            if len(response) > 1:
                self._logger.warning(
                    "Expected 1 node created. Got: {}.\n'{}'".format(
                        len(response), pprint.pformat(response)
                    )
                )
            ...
            tags = self._collect_tag_values_from_port_list(ports)
            self._create_cc_tags(tags, existing_tags, cc_client)

            node_uuid = response[0]['data']['uuid']
            self._create_cc_ports(
                ports, existing_ports, node_uuid, node['name'], cc_client
            )

        return created_updated_nodes

    def _create_cc_node(self, node, existing_nodes, cc_client):
        ...
        existing_node = self._find_resource_from_list(
    ...
    def _find_resource_from_list(self, resources, resource_fq_name):
        for r in resources:
            if not isinstance(r, collections.Mapping):
                continue
            if not "fq_name" in r:
                continue
            if r["fq_name"] == resource_fq_name:
                return r
        return None
    ...
    def _collect_tag_values_from_port_list(self, port_list):
        collected_tags = []

        for p in port_list:
            if not isinstance(p, collections.Mapping) or "tags" not in p:
                continue
            tags = p.get('tags',[])
            for t in tags:
    ...
    def _create_cc_tags(self, tag_values, existing_tags, cc_client):
        tags_to_create = []
        tag_values = set(tag_values)

        for tag_value in tag_values:
            existing_tag = self._find_resource_from_list(
                existing_tags, ["label={}".format(tag_value)]
            )
            if existing_tag is None:
                tags_to_create.append(self.get_cc_tag_payload(tag_value))
