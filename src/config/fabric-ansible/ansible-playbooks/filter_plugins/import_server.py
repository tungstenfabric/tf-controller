...
class FilterModule(object):
    ...
    def convert_port_format(
        ...
        for p in port_list:
            ...
            if 'tags' in p and len(p['tags']) > 0:
                p['tag_refs'] = self._tag_values_to_tag_refs(p['tags'])
    ...
    def _unwrap_list_of_nested_resources(self, list_of_nested_resources):
        """
        ...
        """
        if not isinstance(list_of_nested_resources, list):
            self._logger.error(
                "Expected list of nested resources. Got: {}.".format(
                    pprint.pformat(list_of_nested_resources)
                )
            )

        for i in range(len(list_of_nested_resources)):
            keys = list(list_of_nested_resources[i])
            if len(keys) != 1:
                self._logger.error(
                    "Expected nested resource. Got: {}.".format(
                        pprint.pformat(list_of_nested_resources[i])
                    )
                )

            data = list_of_nested_resources[i][keys[0]]
            if not isinstance(data, dict):
                self._logger.error(
                    "Expected dict data object. Got: {}.".format(
                        pprint.pformat(data)
                    )
                )

            list_of_nested_resources[i] = data
