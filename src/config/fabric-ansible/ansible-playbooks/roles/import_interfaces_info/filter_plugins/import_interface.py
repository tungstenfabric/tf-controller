...
class FilterModule(object):
    ...
    def import_interface(cls, pb_input, device_info, ri_info):
        if not pb_input.get('interface_filters', []):
            pb_input.update(
                {"interface_filters": [{'op': 'regex', 'expr': '^xe|^ge|^et|^fxp|^xle|^fc|^fte|^ae|^reth|^lo0'}]})
        regex_list = []
        ...
        cf_interfaces_list = device_info.get('configured_interfaces_info', {}).get('config_parsed', {})\
            .get('configuration', {}).get('interfaces', {}).get('interface', {})
        rt_interfaces_list = device_info.get('runtime_interfaces_info', {}).get('parsed_output', {})\
            .get('interface-information', {}).get('physical-interface', {})
        ri_config = ri_info.get('routing_instances_info', {}).get('config_parsed', {})\
            .get('configuration', {})
        if ri_config == '':
            ri_config = {}
        ...
        for phy_interface in rt_interfaces_list or []:
            if re.search(regex_str or ".*", phy_interface.get('name', '')):
                ...
                if physical_interface_port_id:
                    phy_interface_payload.update({"physical_interface_port_id": physical_interface_port_id})
                physical_interface_mac_address = phy_interface.get('current-physical-address', '')
                if physical_interface_mac_address:
                    phy_interface_payload.update({"physical_interface_mac_address": physical_interface_mac_address})
                    ...
                    for log_unit in log_units:
                        ...
                        if add_fmly:
                            ...
                            for address_fmly in address_fmly_list:
                                if address_fmly.get('address-family-name', '') == "eth-switch":
                                    log_type = "l2"
                            log_interface_payload.update({"logical_interface_type": log_type})
                        rt_log_interfaces_payloads.append(log_interface_payload)
                ...
                if 'unit' in phy_interface:
                    ...
                    for log_unit in log_units:
                        ...
                        if 'family' in log_unit:
                            if 'ethernet-switching' in log_unit.get('family', {}):
                                log_interface_payload.update({"logical_interface_type": "l2"})
                            else:
                                log_interface_payload.update({"logical_interface_type": "l3"})
                                if 'inet' in log_unit.get('family', {}) and 'address' in log_unit.get('family', {}).\
                                        get('inet', {}) and phy_intf_name == 'lo0' \
                                        and not ('lo0.' + unit_name in lo_ri_map):
                                    addr = log_unit.get('family', {}).get('inet', {}).get('address', {})
                                    if isinstance(addr, dict):
                                        lo0_add_list = [addr]
                                    else:
                                        lo0_add_list = addr
                                    for lo0_add in lo0_add_list:
                                        ip = lo0_add.get('name', '').split('/')[0]
                                        if ip != '127.0.0.1':
                                            if unit_name == '0':
                                                if lo0_add.get('primary', None) == '':
                                                    lo0_lb = ip
                                                    break
                                                elif IPAddress(ip) < IPAddress(lo0_lb):
                                                    lo0_lb = ip
                                                    continue
                                            elif lo0_lb == '255.255.255.255':
                                                if lo0_add.get('primary', None) == '':
                                                    if IPAddress(ip) < IPAddress(primary_lb):
                                                        primary_lb = ip
                                                        break
                                                elif IPAddress(ip) < IPAddress(loopback) \
                                                        and primary_lb == '255.255.255.255':
                                                    loopback = ip
                        if not do_not_add_in_payload:
                            cf_log_interfaces_payloads.append(log_interface_payload)
        ...
        if pb_input.get('import_configured'):
            phy_interfaces_list = list(rt_phy_interfaces_payloads) + list(cf_phy_interfaces_payloads)
            log_interfaces_list = list(rt_log_interfaces_payloads) + list(cf_log_interfaces_payloads)
        else:
