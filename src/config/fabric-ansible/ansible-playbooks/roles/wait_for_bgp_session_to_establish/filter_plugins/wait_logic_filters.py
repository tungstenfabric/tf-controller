...
class FilterModule(object):
    ...
    def is_wait_required(self, curr_down_peer_count, health_check_params):
        ...
        health_check_abort = health_check_params.\
            get('health_check_abort', True)
        bgp_down_peer_check = advanced_params_input.\
            get('bgp', {}).get('bgp_down_peer_count_check', True)
        bgp_down_peer_count = advanced_params_input.\
            get('bgp', {}).get('bgp_down_peer_count', 0)
        bgp_peer_state_check = advanced_params_input.\
            get('bgp', {}).get('bgp_peer_state_check', True)
        ...
        # peer_state is true
        #             - then wait for till down_peer_count is zero
        # if health_check_abort is true and down_peer_check is false and
        # peer_state is false
        #             - then no wait logic
        ...
        if health_check_abort:
            if bgp_down_peer_check:
                if int(curr_down_peer_count) in range(0, bgp_down_peer_count + 1):  # noqa: E501
                    return False
                else:
                    return True
