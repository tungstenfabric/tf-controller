...
class DocDiff(object):
    ...
    def _compare_message_dicts(self, old_mdict, new_mdict):
        ...
        content_modified_messages = [
            mname for mname in list(modified_messages.keys()) if
            modified_messages[mname][0]["fingerprint"] !=
            modified_messages[mname][1]["fingerprint"]]
        severity_modified_messages = {
            mname: (modified_messages[mname][0]["severity"],
                    modified_messages[mname][1]["severity"])
            for mname in list(modified_messages.keys()) if
...
def main():
    if len(sys.argv) != 3:
        print("Usage is python contrail-diff-docs.py <path to old ",
              "index_logs.doc.schema.json> <path new ",
              "index_logs.doc.schema.json>")
        exit(-1)
