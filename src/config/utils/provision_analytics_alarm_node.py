...
from vnc_api.vnc_api import AnalyticsAlarmNode
...
class AnalyticsAlarmNodeProvisioner(object):
    ...
    def _parse_args(self, args_str):
        '''
        Eg. python provision_analytics_alarm_node.py
        ...
        parser.add_argument(
            "--host_name",
            help="hostname name of analytics alarm node",
            required=True)
        parser.add_argument(
            "--host_ip",
            help="IP address of analytics alarm node",
            required=True)
    ...
    def add_analytics_alarm_node(self):
        gsc_obj = self._global_system_config_obj

        analytics_alarm_node_obj = AnalyticsAlarmNode(
            self._args.host_name, gsc_obj,
            analytics_alarm_node_ip_address=self._args.host_ip)
        analytics_alarm_node_exists = True
        try:
            analytics_alarm_node_obj = self._vnc_lib.analytics_alarm_node_read(
                fq_name=analytics_alarm_node_obj.get_fq_name())
        except NoIdError:
            analytics_alarm_node_exists = False

        if analytics_alarm_node_exists:
...
# end class AnalyticsAlarmNodeProvisioner
