...
from vnc_api.vnc_api import GracefulRestartParametersType
...
class ControlProvisioner(BgpProvisioner):
        ...
        if not self._args.oper or self._args.oper == 'add':
            try:
                ...
                if not router_obj.get_global_system_config_back_refs():
                    print("global-system-config-bgp-router "
                          "link is not present")
                    return True
    ...
    def provision_control_bgp(self):
        if self._args.oper == 'add':
            if self._args.sub_cluster_name:
                print("Perfroming provisioning of controller "
                      "specific BGP parameters")
                self.add_bgp_router(
    ...
    def llgr_time_type(self, value):
        ...
        if time < 0 or time > 16777215:
            raise argparse.ArgumentTypeError(
                "long_lived_graceful_restart_time %s "
                "must be in range (0..16777215)" %
                value)
        return time

    def _parse_args(self, args_str):
        '''
        Eg. python provision_control.py
        ...
        parser.add_argument(
            ...
            help="Local autonomous-system number used to peer "
                 "contrail-control bgp speakers across different "
