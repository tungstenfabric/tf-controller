...
class TestProvisionVrouter(UtilsTestCase):
    ...
    def test_add_virtual_router(self):
        ...
        prov_args = [
            ...
            "--sriov_physnets %s" % ' '.join([k+'='+v for k,v in physnet_mapping.items()]),
        ]
        ...
        physnets = vrouter_obj.get_virtual_router_sriov_physical_networks() or KeyValuePairs()
        ...
        for physnet, interface in physnet_mapping.items():
            ...
            assert kvp in physnet_kvps, "(%s) kv pair not found in physnet kvps (%s)" % (kvp, physnet_kvps)
