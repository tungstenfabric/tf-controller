...
class ProvisionVgwInterface(object):
    ...
    def __init__(self, args_str=None):
        ...
        if self._args.oper == "create":
            ...
            gw_str = "[{\"interface\":\"%s\", \"routing-instance\":\"%s\", %s, %s}]" % ( # noqa
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument(
            "--subnets", nargs='+',
            help="List of subnets in virtual-network configured for gateway"
                 "(Ex: 1.1.1.0/24 2.2.2.0/24)")
        parser.add_argument(
            "--routes",
            nargs='+',
            help="List of public routes injected into virtual-network"
                 "routing-instance (Ex: 8.8.8.0/24 9.9.9.0/24)")
        parser.add_argument(
            "--vrf",
            help="Routing instance for virtual-network configured for gateway"
                 "(as FQDN)")
