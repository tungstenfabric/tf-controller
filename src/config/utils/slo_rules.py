...
from vnc_api.vnc_api import SecurityLoggingObjectRuleListType
...
class RulesToFromSLO(object):
    def __init__(self, args_str=None):
        ...
        if self._args.rules:
            ...
            for key in rule_dict:
                ...
                entry = SecurityLoggingObjectRuleEntryType(
                    rule_uuid=key, rate=rule_dict[key])
    ...
    def _parse_args(self, args_str):
        '''
        Eg. python slo_rules.py
                --slo_fq_name=default-domain:default-project:slo1
                --rules={\"ff4da0ac-4225-4cdf-a652-65924e1116c3\":10\,\"5600fcb7-6e12-47da-b658-1f6d87710e7b\":20\,\"dccae58e-1ab1-44ca-b12d-e4180a6b7c4f\":30}
            python slo_rules.py
                --slo_fq_name=default-domain:default-project:slo1
                --sg_fq_name=default-domain:demo:default --rate=20
        '''
        ...
        parser.add_argument(
            ...
            choices=[
                'associate',
                'disassociate'],
            help="operation type - associate/disassociate network-policy/SG with SLO")
        parser.add_argument(
            "--slo_fq_name",
        ...
        parser.add_argument(
            "--sg_fq_name",
            help="Fully qualified name of Security Group object")
        parser.add_argument(
            "--policy_fq_name",
            help="Fully qualified name of Network policy object")
        parser.add_argument(
            "--rules",
            help="json as {\"rule-uuid2\": rate1, \"rule-uuid2\": rate2}")
        ...
        if self._args.sg_fq_name and self._args.policy_fq_name:
            print(
                "The arguments --sg_fq_name and --policy_fq_name cannot be specified together")
            ret_value = False
        ...
        if self._args.rules and self._args.policy_fq_name:
            print(
                "The arguments --rules and --policy_fq_name cannot be specified together")
            ret_value = False
    ...
    def UpdateSLORules(self, slo, rule_list_obj):
        if rule_list_obj:
            ...
            for rule in rule_list:
                ...
                # Create SLO rule
                entry = SecurityLoggingObjectRuleEntryType(
                    rule_uuid=rule.get_rule_uuid(), rate=self._args.rate)
    ...
    def AssociateSgRules(self, slo):
        try:
            sg = self._vnc_lib.security_group_read(
                fq_name=None, fq_name_str=self._args.sg_fq_name)
        except NoIdError:
            print("security-group %s does NOT exist" % (self._args.sg_fq_name))
            return
        # create empty SLO rule list
        slo_rule_list = SecurityLoggingObjectRuleListType()
        # Add SG to SLO. This will add all the rules from ACLs generated for SG
        # to SLO
    ...
    def AssociatePolicyRules(self, slo):
        try:
            pol = self._vnc_lib.network_policy_read(
                fq_name=None, fq_name_str=self._args.policy_fq_name)
        except NoIdError:
            print("Network policy %s does NOT exist" %
                  (self._args.policy_fq_name))
            return
        # create empty SLO rule list
        slo_rule_list = SecurityLoggingObjectRuleListType()
        # Add NetworkPolicy to SLO. This will add all the rules from ACLs
        # generated for NetworkPolicy to SLO
