...
from cfgm_common.exceptions import (
        ResourceExhaustionError, NoIdError, RefsExistError)
from vnc_api.vnc_api import (
        SubnetType, SubnetListType, VirtualRouter,
        SubCluster, VirtualMachineInterface)
        ...
        for value in values:
            try:
                ...
                raise argparse.ArgumentError(
                        self, f"could not parse argument \"{value}\"")
...
class VrouterProvisioner(object):
    ...
    def add_vrouter(self):
        ...
        if self._args.sriov_physnets:
            self.set_virtual_router_sriov_physical_networks(
