...
#tenant_fqname = ['default-domain', 'admin']
...
class ProvisionInfraNetwork(object):
        ...
        for network, details in self.connections.items():
            ...
            for server, routers in details['servers'].items():
                if server in servers:
                    msg = 'Server %s is mentioned more than once'%server
                    #raise Exception(msg)
                servers.add(server)
                for router, pifs in routers.items():
                    for pif in pifs:
                        physical_interface = ":".join([router, pif])
                        if physical_interface in physical_interfaces:
                            msg = 'Physical Interface %s is mentioned more than once'%(physical_interface)
                            #raise Exception(msg)
            ...
            for server, routers in details['servers'].items():
                ...
                for router, pifs in routers.items():
                    for pif in pifs:
                        ...
                        physical_interfaces.add(physical_interface)
                    prouters.add(router)
                self.check_and_create_vpg(server, physical_interfaces, network, data_network)
    ...
    def check_and_create_vpg(self, server, pifs, network, data_network):
        for pif in pifs:
            if not data_network:
                ...
                native_vlan = self.vlans[network]
    ...
    def create_port(self, name, network, vpg_name, pifs, native_vlan=None):
        fq_name = tenant_fqname + [name]
        try:
