...
class CreateDeleteSLO(object):
        ...
        try:
            ...
            if self._args.parent == 'global-vrouter':
                parent = self._vnc_lib.global_vrouter_config_read(
                    fq_name=None, fq_name_str="default-global-system-config:default-global-vrouter-config")
            else:
                parent = self._vnc_lib.project_read(
                    fq_name=None, fq_name_str="default-domain:default-project")
            slo = SecurityLoggingObject(name=self._args.name, parent_obj=parent,
                                        security_logging_object_rules=None, security_logging_object_rate=self._args.rate)
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument("--oper", choices=['add', 'delete', 'associate', 'disassociate'],
                            help="operation type - create/remove/associate/disassociate SLO", required=True)
        parser.add_argument(
            "--parent", choices=['global-vrouter', 'default-project'], help="Parent object type")
        parser.add_argument("--rate", type=int, default=100,
                            help="Every 'rate' th flow matching SLO is logged")
        parser.add_argument(
            "--slo_fq_name", help="Fully qualified name of Security logging object")
        parser.add_argument(
            "--vn_fq_name", help="Fully qualified name of Virtual Network object")
        parser.add_argument(
            "--vmi_fq_name", help="Fully qualified name of Virtual Machine Interface object")
        parser.add_argument("--fw_rule_fq_name",
                            help="Fully qualified name of Firewall rule object")
        parser.add_argument(
            "--fw_policy_fq_name", help="Fully qualified name of Firewall policy object")
        ...
        if self._args.oper == 'associate' or self._args.oper == 'disassociate':
            ...
            if not self._args.vn_fq_name and not self._args.vmi_fq_name and not self._args.fw_rule_fq_name and \
                ...
                print(
                    "VN/VMI/Firewall-rule/Firewall-policy FQ-name required for --oper=associate/disassociate")
                ret_value = False
        else:
