...
Options:
    ...
    -i          Input file which has vrouter and control node details.
                ...
                Specify deployment_method from rhosp_director
                '/'ansible'/'kubernetes'
...
class Debug(object):
    ...
    def __init__(self, dir_name, sub_dirs, process_name, container_name,
        ...
        self._ssh_client = self.create_ssh_connection(
            self._host, self._user, self._pwd, self._ssh_key_file)
    ...
    def copy_introspect(self):
        ...
        for i in range(len(cmd_list)):
            ...
            cmd = sudo_prefix + 'docker exec %s %s %s' % (
                self._container, self._cli, cmd_list[i])
    ...
    def run_command_interval_times(self, cmd, interval, times):
        for i in range(times):
            ...
            myCmd = sudo_prefix + 'timeout %ds docker exec %s /bin/sh -c '\
                '"%s"' % (interval, self._container, cmd)
            cmd_op = self.get_ssh_cmd_output(myCmd)
            time.sleep(2)
            if not cmd_op:
                print('No output for the command %s' % myCmd)
...
def collect_vrouter_node_logs(data):
    ...
    for item in data['provider_config']['vrouter']:
        ...
        obj = Debug(
            ...
            container_name=deployment_map[
                deployment_method][vim_type]['container_name'],
