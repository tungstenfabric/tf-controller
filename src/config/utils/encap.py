...
from vnc_api.vnc_api import GlobalVrouterConfig
from vnc_api.vnc_api import EncapsulationPrioritiesType


if len(sys.argv) < 6:
    print('Usage: python encap_set.py <add|update|delete> '
          '<username> <password> <tenant_name> <config_node_ip>')
    sys.exit(0)
...
if __name__ == "__main__":
    handle = VncApi(
        ...
        api_server_host=sys.argv[5],
        api_server_port='8082',
        api_server_use_ssl=sys.argv[6])
    encap_obj = EncapsulationPrioritiesType(
        encapsulation=['MPLSoGRE', 'MPLSoUDP', 'VXLAN'])
    conf_obj = GlobalVrouterConfig(
        encapsulation_priorities=encap_obj, evpn_status='true')
