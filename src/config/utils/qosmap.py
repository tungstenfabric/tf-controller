...
class QosmapProv(object):
    ...
    def execute_qosmap_cmd(self):
        ...
        for intf in self.ifname_list:
            qos_cmd = ('/usr/bin/qosmap --set-queue ' + intf +
                       ' --dcbx ' + self.dcbx)
            qos_cmd = (qos_cmd + ' --bw ' + self.bandwidth +
                       ' --strict ' + self.scheduling)
        ...
        for intf in intf_list:
            ...
            if (num_tx_queues and num_tx_queues != '0'):
                for i in range(int(num_tx_queues)):
                    ...
                    if (not xps_cpu_file):
                        print("xps_cpu file not found %s on compute %s"
                              "while disabling Xmit-Packet-Steering" % (
                                  xps_cpu_file, compute_host_string)) # noqa
                        return True
