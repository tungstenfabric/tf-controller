...
from vnc_api.vnc_api import ConfigDatabaseNode
...
class ConfigDatabaseNodeProvisioner(object):
    ...
    def _parse_args(self, args_str):
        '''
        Eg. python provision_config_database_node.py
        ...
        parser.add_argument(
            "--host_name",
            help="hostname name of config database node",
            required=True)
        parser.add_argument(
            "--host_ip",
            help="IP address of config database node",
            required=True)
    ...
    def add_config_database_node(self):
        gsc_obj = self._global_system_config_obj

        config_database_node_obj = ConfigDatabaseNode(
            self._args.host_name, gsc_obj,
            config_database_node_ip_address=self._args.host_ip)
        config_database_node_exists = True
        try:
            config_database_node_obj = self._vnc_lib.config_database_node_read(
                fq_name=config_database_node_obj.get_fq_name())
        except NoIdError:
            config_database_node_exists = False

        if config_database_node_exists:
...
# end class ConfigDatabaseNodeProvisioner
