...
from vnc_api.vnc_api import IpamSubnets
from vnc_api.vnc_api import SubnetType
from vnc_api.vnc_api import NetworkIpam
from vnc_api.vnc_api import AllocationPoolType
from vnc_api.vnc_api import VirtualRouterNetworkIpamType
...
class VrouterIpam(object):
    ...
    def _parse_args(self, args_str):
        '''
        Eg. python vrouter_ipam.py
                    --name ipam1 --oper=add --prefix="20.0.0.0" --prefix_len=16
            python vrouter_ipam.py --name ipam1 --oper=delete
            python vrouter_ipam.py --name ipam1 --oper=associate
                    --vrouter_name="default-global-system-config:nodea28"
                    --prefix="20.0.1.0" --prefix_len=24
            python vrouter_ipam.py --name ipam1 --oper=disassociate
                    --vrouter_name="default-global-system-config:nodea28"
        '''
        ...
        parser.add_argument(
            "--name", help="Name of Network-Ipam", required=True)
        parser.add_argument(
            "--oper",
            choices=[
                'add',
                ...
                'disassociate'],
            help="operation type - "
                 "create/remove/associate/disassociate Network-Ipam",
            required=True)
        parser.add_argument(
            "--vrouter_name",
            help="Fully qualified name of virtual-router object")
        parser.add_argument(
            "--prefix", help="Subnet in dotted decimal notation")
        parser.add_argument("--prefix_len", type=int,
                            help="Subnet prefix length")
        ...
        if self._args.oper == 'associate' or self._args.oper == 'disassociate':
            if not self._args.vrouter_name:
                print("virtual-router FQ-name required for "
                      "--oper=associate/disassociate")
                ret_value = False
        else:
    ...
    def HandleAdd(self):
        ...
        try:
            ipam_obj = self._vnc_lib.network_ipam_read(
                fq_name=None, fq_name_str=ipam_fq_name)
        except NoIdError:
            pass
        ...
        ipam_subnets_obj.add_subnets(IpamSubnetType(
            SubnetType(self._args.prefix, self._args.prefix_len)))
        ipam_obj = NetworkIpam(
            name=self._args.name,
            parent_obj=None,
            network_ipam_mgmt=None,
            ipam_subnets=ipam_subnets_obj,
    ...
    def HandleAssociate(self):
        ...
        try:
            vr_obj = self._vnc_lib.virtual_router_read(
                fq_name=None, fq_name_str=self._args.vrouter_name)
        except NoIdError:
            print("Vrouter %s does not exist" % (self._args.vrouter_name))
            return
        pool_data = AllocationPoolType(
            start=None, end=None, vrouter_specific_pool=True)
        subnet_data = SubnetType(self._args.prefix, self._args.prefix_len)
        data = VirtualRouterNetworkIpamType(
            allocation_pools=[pool_data], subnet=[subnet_data])
