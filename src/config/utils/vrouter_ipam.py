...
class VrouterIpam(object):
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument("--oper", choices=['add', 'delete', 'associate', 'disassociate'],
                            help="operation type - create/remove/associate/disassociate Network-Ipam", required=True)
        parser.add_argument(
            "--vrouter_name", help="Fully qualified name of virtual-router object")
        parser.add_argument(
        ...
        if self._args.oper == 'associate' or self._args.oper == 'disassociate':
            if not self._args.vrouter_name:
                print(
                    "virtual-router FQ-name required for --oper=associate/disassociate")
                ret_value = False
        else:
    ...
    def HandleAdd(self):
        ...
        ipam_obj = NetworkIpam(name=self._args.name, parent_obj=None, network_ipam_mgmt=None,
                               ipam_subnets=ipam_subnets_obj, ipam_subnet_method="flat-subnet")
