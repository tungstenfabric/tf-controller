...
protected:
    NdpEntryTest() :
        trigger_(boost::bind(&NdpEntryTest::AddVhostRcvRoute, this),
                         ...
                         0) {
        ip1 = Ip6Address::from_string("::ffff:10.1.1.1");
        //Add a vhost rcv route and check that unsolicited na entry gets created
        TriggerAddVhostRcvRoute(ip1);
        client->WaitForIdle();
        const VrfEntry *vrf = Agent::GetInstance()->vrf_table()->FindVrfFromName(
            Agent::GetInstance()->fabric_vrf_name());
        NdpKey key(ip1, vrf);
        EXPECT_TRUE(Agent::GetInstance()->icmpv6_proto()->FindUnsolNaEntry(key) != NULL);
        sm_ = Agent::GetInstance()->icmpv6_proto()->FindUnsolNaEntry(key);
    }
...
#define TRANSITION(F, E) \
    ...
    Transitions reachable = map_list_of
            ...
            TRANSITION(EvRetransmitTimerExpired, NdpEntry::STALE);
