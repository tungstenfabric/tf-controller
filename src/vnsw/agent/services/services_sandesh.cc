...
void ServicesSandesh::BfdStatsSandesh(std::string ctxt, bool more) {
    ...
    bfd->set_bfd_receive_ka_enqueue_count(stats.bfd_rx_ka_enqueue_count);
...
void InterfaceIgmpStatsReq::HandleRequest() const {
    ...
    IgmpStats rx_badpacket_stats;
    IgmpStats rx_okpacket_stats;
    IgmpInfo::IgmpItfStats stats;

    if (vn_uuid.empty() || gw.empty()) {
        SandeshError("Empty vn_uuid or gateway !!", context());
        return;
    }
    ...
    rx_badpacket_stats.undefined =
        stats.rx_badpacket[IGMP_UNDEFINED - 1];
    rx_badpacket_stats.igmp_membership_query =
        stats.rx_badpacket[IGMP_MEMBERSHIP_QUERY - 1];
    rx_badpacket_stats.igmp_v1_membership_report =
        stats.rx_badpacket[IGMP_V1_MEMBERSHIP_REPORT - 1];
    rx_badpacket_stats.igmp_proto_dvmrp =
        stats.rx_badpacket[IGMP_PROTO_DVMRP - 1];
    rx_badpacket_stats.igmp_proto_pim =
        stats.rx_badpacket[IGMP_PROTO_PIM - 1];
    rx_badpacket_stats.igmp_cisco_trace =
        stats.rx_badpacket[IGMP_CISCO_TRACE - 1];
    rx_badpacket_stats.igmp_v2_membership_report =
        stats.rx_badpacket[IGMP_V2_MEMBERSHIP_REPORT - 1];
    rx_badpacket_stats.igmp_group_leave =
        stats.rx_badpacket[IGMP_GROUP_LEAVE - 1];
    rx_badpacket_stats.igmp_mtrace_response =
        stats.rx_badpacket[IGMP_MTRACE_RESPONSE - 1];
    rx_badpacket_stats.igmp_mtrace_request =
        stats.rx_badpacket[IGMP_MTRACE_REQUEST - 1];
    rx_badpacket_stats.igmp_dwr =
        stats.rx_badpacket[IGMP_DWR - 1];
    rx_badpacket_stats.igmp_v3_membership_report =
        stats.rx_badpacket[IGMP_V3_MEMBERSHIP_REPORT - 1];
    igmp->set_rx_badpacket_stats(rx_badpacket_stats);

    rx_okpacket_stats.undefined =
        stats.rx_okpacket[IGMP_UNDEFINED - 1];
    rx_okpacket_stats.igmp_membership_query =
        stats.rx_okpacket[IGMP_MEMBERSHIP_QUERY - 1];
    rx_okpacket_stats.igmp_v1_membership_report =
        stats.rx_okpacket[IGMP_V1_MEMBERSHIP_REPORT - 1];
    rx_okpacket_stats.igmp_proto_dvmrp =
        stats.rx_okpacket[IGMP_PROTO_DVMRP - 1];
    rx_okpacket_stats.igmp_proto_pim =
        stats.rx_okpacket[IGMP_PROTO_PIM - 1];
    rx_okpacket_stats.igmp_cisco_trace =
        stats.rx_okpacket[IGMP_CISCO_TRACE - 1];
    rx_okpacket_stats.igmp_v2_membership_report =
        stats.rx_okpacket[IGMP_V2_MEMBERSHIP_REPORT - 1];
    rx_okpacket_stats.igmp_group_leave =
        stats.rx_okpacket[IGMP_GROUP_LEAVE - 1];
    rx_okpacket_stats.igmp_mtrace_response =
        stats.rx_okpacket[IGMP_MTRACE_RESPONSE - 1];
    rx_okpacket_stats.igmp_mtrace_request =
        stats.rx_okpacket[IGMP_MTRACE_REQUEST - 1];
    rx_okpacket_stats.igmp_dwr =
        stats.rx_okpacket[IGMP_DWR - 1];
    rx_okpacket_stats.igmp_v3_membership_report =
        stats.rx_okpacket[IGMP_V3_MEMBERSHIP_REPORT - 1];
...
bool ArpSandesh::SetArpEntry(const ArpKey &key, const ArpEntry *entry) {
    ...
    if (entry->get_interface())
        data.set_interface_name(entry->get_interface()->name());
    std::string mac_str;
