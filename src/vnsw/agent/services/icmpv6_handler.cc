...
bool Icmpv6Handler::HandleMessage() {
    ...
    switch(pkt_info_->ipc->cmd) {
        case Icmpv6Proto::NDP_RESOLVE: {
            ...
            //icmp_proto->IncrementStatsNdpReq();
            //icmp_proto->IncrementStatsNdpRequest(ipc->interface->id());
            break;
        }
...
void Icmpv6Handler::SendNeighborSolicit(const Ip6Address &sip,
    ...
    uint16_t len = FillNeighborSolicit((uint8_t *)icmp_, tip, source_ip,
                                       solicited_mcast_ip);
    SendIcmpv6Response(vmi?vmi->id():0, vrf, source_ip,
