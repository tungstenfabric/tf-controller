...
protected:
    ...
    void AddGatewayRoute(const std::string &vrf_name,
        ...
        agent_->fabric_inet4_unicast_table()->AddGatewayRouteReq
            (agent_->local_peer(), vrf_name, ip, plen, AddressList(1, server), vn_list,
...
TEST_F(RouteTest, RemoteVmRoute_5) {
    if (!is_gateway_configured) {
        agent_->set_vhost_default_gateway(AddressList(1, fabric_gw_ip_));
    ...
    if (!is_gateway_configured) {
        agent_->set_vhost_default_gateway(AddressList(1, default_dest_ip_));
        AddGatewayRoute(agent_->fabric_vrf_name(), default_dest_ip_, 0,
                        default_dest_ip_);
        client->WaitForIdle();
    }
...
TEST_F(RouteTest, RemoteVmRoute_foreign_gw) {
    agent_->set_vhost_default_gateway(AddressList(1, foreign_gw_ip_));
    AddGatewayRoute(agent_->fabric_vrf_name(), default_dest_ip_, 0,
                    foreign_gw_ip_);
    client->WaitForIdle();
...
// Tests if AddEvpnEcmpRoute adds route with vn info
TEST_F(RouteTest, evpn_ecmp_type5_add_remote_route24) {
    using boost::uuids::nil_uuid;
    struct PortInfo input1[] = {
        {"vnet1", 1, "1.1.1.10", "00:00:01:01:01:10", 1, 1},
    };
    ...
    item.entry.nlri.af = BgpAf::L2Vpn;
    item.entry.nlri.safi = BgpAf::Enet;
    item.entry.nlri.address="3.3.3.0/24";
    item.entry.nlri.ethernet_tag = 0;
    autogen::EnetNextHopType nh1, nh2;
    nh1.af = Address::INET;
    nh1.address = "10.0.0.24";
    ...
    bgp_peer_->GetAgentXmppChannel()->AddEvpnEcmpRoute(routing_vrf_name,
                      MacAddress::FromString( "00:00:00:00:00:00"),
                      Ip4Address::from_string("3.3.3.0"), 24, &item, vn_list);
    client->WaitForIdle();

    EvpnRouteEntry *evpn_rt = NULL; //EvpnRouteGet(routing_vrf_name,
                                   //MacAddress::FromString("00:00:00:00:00:00"),
                                   //Ip4Address::from_string("3.3.3.0"), 0);
    {
        VrfEntry *evpn_vrf = agent_->vrf_table()->
            FindVrfFromName(routing_vrf_name);
        EXPECT_TRUE(evpn_vrf != NULL);

        EvpnRouteKey key(agent_->local_vm_peer(),
            routing_vrf_name, 
            MacAddress::FromString("00:00:00:00:00:00"),
        ...
        evpn_rt =
            ...
            (evpn_vrf->GetEvpnRouteTable())->FindActiveEntry(&key));
    }
    client->WaitForIdle();
    EXPECT_TRUE(evpn_rt != NULL);
    EXPECT_TRUE(evpn_rt->GetActivePath()->dest_vn_list() == vn_list);
    ...
    InetUnicastRouteEntry *inet_rt =
        RouteGet(routing_vrf_name, Ip4Address::from_string("3.3.3.0"), 24);
    client->WaitForIdle();
    EXPECT_TRUE(inet_rt != NULL); // /24 route should leak
