...
void AgentParam::BuildAddrList(const string &val, AddressList& addr_list) {
    addr_list.clear();
    if (val.empty()) {
        return;
    }
    ...
    while (it != tokens.end()) {
        ...
        if (str.find('/') != std::string::npos) {
            ...
            if (ec != 0 || vhost_.plen_ >= 32) {
                ...
                eth_port_plen_list_.push_back(plen);
            }
        } else {
            if (GetIpAddress(str, &addr)) {
                addr_list.push_back(addr);
...
void AgentParam::ParseVirtualHostArguments
    ...
    string eth_port_addr_list;
    if (GetOptValue<string>(var_map, eth_port_addr_list,
                "VIRTUAL-HOST-INTERFACE.physical_interface_addr")) {
        BuildAddrList(eth_port_addr_list, eth_port_addr_list_);
    }

    string gateway_list;
    if (GetOptValue<string>(var_map, gateway_list,
                "VIRTUAL-HOST-INTERFACE.gateway")) {
        BuildAddrList(gateway_list, gateway_list_);
    }
...
AgentParam::AgentParam(bool enable_flow_options,
    ...
    if (enable_vhost_options_) {
        ...
        vhost.add_options()
            ...
            ("VIRTUAL-HOST-INTERFACE.gateway",
             opt::value<string>(),
             "Lisg of gateway IP address for virtual host")
            ("VIRTUAL-HOST-INTERFACE.physical_interface",
             opt::value<std::vector<std::string> >()->multitoken(),
             "Physical interface name to which virtual host interface maps to")
            ("VIRTUAL-HOST-INTERFACE.physical_interface_addr",
             opt::value<string>(),
             "List of Physical interface ip address")
