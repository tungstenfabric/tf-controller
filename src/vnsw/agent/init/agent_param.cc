...
void AgentParam::BuildEthPortAddrList(const string &val) {
    eth_port_addr_list_.clear();
    if (val.empty()) {
        return;
    }
    ...
    while (it != tokens.end()) {
        ...
        Ip4Address addr;
        if (GetIpAddress(str, &addr)) {
            eth_port_addr_list_.push_back(addr);
        } else {
            LOG(ERROR, "Error in parsing address " << *it);
        }
    }
...
void AgentParam::ParseVirtualHostArguments
    ...
    if (GetOptValue<string>(var_map, eth_port_addr_list,
        ...
        BuildEthPortAddrList(eth_port_addr_list);
    }
