...
void ContrailInitCommon::CreateInterfaces() {
    ...
    for (std::size_t i = 0; i != agent_param()->eth_port_list().size(); ++i) {
        ...
        if (agent()->is_l3mh()) {
            // Receive route for physical interface IP address assigned
            rt_table->AddVHostRecvRouteReq(agent()->local_peer(), agent()->fabric_vrf_name(), physical_key,
                             agent_param()->eth_port_addr_list()[i], 32, agent()->fabric_vn_name(), false, true);
            // Receive route for the sub-net broadcast address
            rt_table->AddVHostSubnetRecvRoute(agent()->local_peer(), agent()->fabric_vrf_name(),
                                   physical_key,
                                   GetIp4SubnetBroadcastAddress(agent_param()->eth_port_addr_list()[i], agent_param()->eth_port_plen_list()[i]),
                                   32, agent()->fabric_vn_name(), false);
            // Add resolve route for physical interface ip-addr to vrf 0
            rt_table->AddResolveRoute(agent()->local_peer(),
                                      agent()->fabric_vrf_name(),
                                      agent_param()->eth_port_addr_list()[i],
                                      agent_param()->eth_port_plen_list()[i],
                                      physical_key, MplsTable::kInvalidLabel,
                                      false, agent()->fabric_vn_name(),
                                      SecurityGroupList(), TagList());
        }
    }

    if (agent()->is_l3mh()) {
        agent()->set_router_id(agent_param()->loopback_ip());
    } else {
