...
VmInterface::VmInterface(const boost::uuids::uuid &uuid,
    ...
    dhcp_enable_(true), dhcp_enable_v6_(false), do_dhcp_relay_(false), proxy_arp_mode_(PROXY_ARP_NONE),
...
bool VmInterface::Resync(const InterfaceTable *table,
    ...
    if (dhcp_enabled_ || dhcp_enable_v6_)
        dns_enabled_ = true;
    else
        dns_enabled_ = false;
...
bool MacVmBindingState::DeleteL3(const Agent *agent, VmInterface *vmi) const {
    ...
    // return if table is already marked for deletion, bridge route delete might
    // already be processed in ReComputePathDeletion
    if (table == NULL)
        return true;
