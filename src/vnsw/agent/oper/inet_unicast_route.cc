...
bool Inet4UnicastGatewayRoute::AddChangePathExtended(Agent *agent, AgentPath *path,
    ...
    if (gw_list_.empty() || gw_list_.size() == 1) { /* SH case */
        ...
        AddressList::const_iterator ptr;
        for (ptr = gw_list_.begin(); ptr < gw_list_.end(); ptr++) {
            InetUnicastRouteEntry *uc_rt = table->FindRoute(*ptr);
            rt = uc_rt;
            if (uc_rt == NULL || uc_rt->plen() == 0) {
                ...
                InetUnicastAgentRouteTable::AddArpReq(vrf_name_, *ptr,
                        nexthop_vrf, nh->get_interface(), nh->PolicyEnabled(),
                        vn_list_, sg_list_, tag_list_);
            } else {
        ...
        if (comp_nh_list.size() == gw_list_.size()) {
            path->set_unresolved(false);
    ...
    if (agent->is_l3mh() && (path->unresolved() == false) &&
            path->ChangeNH(agent, nh) == true) {
        return true;
    }
