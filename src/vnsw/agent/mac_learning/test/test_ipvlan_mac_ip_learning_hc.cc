...
TEST_F(MacIpLearningHCTest, BfdTest1) {
    const VmInterface *intf = static_cast<const VmInterface *>(VmPortGet(1));
    TxL2Packet(intf->id(), intf->vm_mac().ToString().c_str(), "00:00:00:11:22:31",
               "1.1.1.10", "1.1.1.11", 1, intf->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();

    intf = static_cast<const VmInterface *>(VmPortGet(2));
    TxL2Packet(intf->id(), intf->vm_mac().ToString().c_str(), "00:00:00:11:22:30",
...
TEST_F(MacIpLearningHCTest, BfdTest5) {
    ...
    intf = static_cast<const VmInterface *>(VmPortGet(2));
    MacAddress smac = intf->vm_mac();
    TxL2Packet(intf->id(), smac.ToString().c_str(), "00:00:00:11:22:30",
               "1.1.1.10", "1.1.1.11", 1, intf->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
