...
// send trap for multiple IPVLAN mac-ip learning and verify learnt routes
TEST_F(MacIpLearningTest, Test1) {
    const VmInterface *intf = static_cast<const VmInterface *>(VmPortGet(1));
    MacAddress smac = intf->vm_mac();
    ...
    TxL2Packet(intf->id(), smac.ToString().c_str(), "00:00:00:33:22:11",
               "1.1.1.4", "1.1.1.11", 1, intf->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
    IpAddress s_sip = AddressFromString("1.1.1.4", &error_code);
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, Ip4Address(0), 0) != NULL);
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, AddressFromString("1.1.1.3", &error_code), 0) != NULL);
    EXPECT_TRUE(L2RouteGet("vrf1", smac) != NULL);
    EXPECT_TRUE(RouteGet("vrf1", s_sip.to_v4(), 32) != NULL);
    nh = dynamic_cast<const InterfaceNH *>(L2RouteToNextHop("vrf1", smac));
    EXPECT_TRUE(nh->GetDMac() == smac);
    EXPECT_TRUE(nh->PolicyEnabled());
    nh = dynamic_cast<const InterfaceNH *>(RouteToNextHop("vrf1", s_sip.to_v4(), 32));
    EXPECT_TRUE(nh->GetDMac() == smac);
    EXPECT_TRUE(nh->PolicyEnabled());
    ...
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, AddressFromString("1.1.1.4", &error_code), 0) == NULL);
    EXPECT_TRUE(RouteGet("vrf1", s_sip.to_v4(), 32) == NULL);
}
// if IPVLAN target IP is unreachable , verify that routes of target IP are deleted.
TEST_F(MacIpLearningTest, Test2) {
    ...
    client->WaitForIdle();
    // will be removed only during parent intf delete or admin down
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, Ip4Address(0), 0) != NULL);
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, AddressFromString("1.1.1.3", &error_code), 0) == NULL);
    // will be removed only during parent intf delete or admin down
    EXPECT_TRUE(L2RouteGet("vrf1", smac) != NULL);
...
TEST_F(MacIpLearningTest, Test3) {
    const VmInterface *intf_old = static_cast<const VmInterface *>(VmPortGet(1));
    MacAddress smac_old = intf_old->vm_mac();
    TxL2Packet(intf_old->id(), smac_old.ToString().c_str(), "00:00:00:33:22:11",
               "1.1.1.3", "1.1.1.11", 1, intf_old->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
    boost::system::error_code error_code;
    IpAddress sip = AddressFromString("1.1.1.3", &error_code);
    ...
    EXPECT_TRUE(agent_->mac_learning_proto()->GetMacIpLearningTable()->
            GetPairedMacAddress(intf_old->vrf()->vrf_id(), sip) == smac_old);

    const VmInterface *intf_new = static_cast<const VmInterface *>(VmPortGet(2));
    MacAddress smac_new = intf_new->vm_mac();
    TxL2Packet(intf_new->id(), smac_new.ToString().c_str(), "00:00:00:33:22:11",
               "1.1.1.3", "1.1.1.11", 1, intf_new->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
    ...
    EXPECT_TRUE(agent_->mac_learning_proto()->GetMacIpLearningTable()->
            GetPairedMacAddress(intf_new->vrf()->vrf_id(), sip) == smac_new);
    const InterfaceNH *nh  =
