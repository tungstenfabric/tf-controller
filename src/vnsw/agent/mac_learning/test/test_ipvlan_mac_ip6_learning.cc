...
struct PortInfo input[] = {
    ...
    {"intf2", 2, "8.1.1.2", "00:00:00:01:01:02", 1, 2, "fd11::3"}
};
...
public:
    ...
    virtual void TearDown() {
        DeleteVmportEnv(input, 2, true, 0, NULL, NULL, true, true);
        client->WaitForIdle();
...
TEST_F(MacIp6LearningTest, Test2) {
    ...
    client->WaitForIdle();
    // will be deleted only if vmi is deleted
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, IpAddress(), 0) != NULL);
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, AddressFromString("fd11::5", &error_code), 0) == NULL);
    // will be deleted only if vmi is deleted
    EXPECT_TRUE(L2RouteGet("vrf1", smac) != NULL);
...
TEST_F(MacIp6LearningTest, Test3) {
    const VmInterface *intf = static_cast<const VmInterface *>(VmPortGet(1));
    MacAddress smac_old = intf->vm_mac();
    TxL2Packet(intf->id(), smac_old.ToString().c_str(), "00:00:00:33:22:11",
               "fd11::5", "fd11::1", 58, intf->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
    boost::system::error_code error_code;
    Ip6Address sip = Ip6Address::from_string("fd11::5", error_code);
    ...
    TxL2Packet(intf_new->id(), smac_new.ToString().c_str(), "00:00:00:33:22:11",
               "fd11::5", "fd11::1", 58, intf_new->vrf()->vrf_id(), 1, 1);
    client->WaitForIdle();
