...
BgpXmppChannel::~BgpXmppChannel() {
    ...
    channel_->UnRegisterReferer(peer_id_);
...
bool BgpXmppChannel::ProcessItem(string vrf_name,
    ...
    // - Label must be 0 unless it is INETMPLS
...
void BgpXmppChannelManager::XmppHandleChannelEvent(XmppChannel *channel,
    ...
    if (state == xmps::READY) {
        ...
        if (it != channel_map_.end()) {
            ...
            ostringstream os;
            os << "Peer not found for " << channel->ToString() <<
                  " on channel down event";
            BGP_LOG_NOTICE(BgpMessage, BGP_LOG_FLAG_ALL, os.str());
        }
    }
}
